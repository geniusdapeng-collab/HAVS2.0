...
const runFile = fs.readFileSync('./run-wukong-erlang-preproduction.js', 'utf-8');
const match = runFile.match(/const engine = new ProductionEngine\(\{[\s\S]*?\}\);/);
if (!match) {
  console.error('Failed to extract engine config');
  process.exit(1);
}

const engineConfig = match[0].replace(
  /budget:\s*\d+/, 
  `budget: 3600`
);
...
async function main() {
  const engine = ${engineConfig.replace('const engine = ', '').replace('new ProductionEngine', 'new ProductionEngine').replace('};', '')};
...
fs.writeFileSync('./resume-wukong-phase3.js', script);
console.log('续跑脚本已生成: ./resume-wukong-phase3.js');
