...
function 关键字段存在(prompt) {
  return {
    ...
    // v6.6.3-fix: 新增关键字段
    情绪: prompt.includes('【情绪】') || /MOOD\s*:/i.test(prompt),
    运镜: prompt.includes('【运镜】') || /CAMERA\s*:/i.test(prompt),
    灯光: prompt.includes('【灯光】') || /LIGHTING\s*:/i.test(prompt),
    渲染参数: prompt.includes('【渲染参数】') || /RENDER\s*:/i.test(prompt),
    导演指令: prompt.includes('【导演指令】') || /DIRECTOR\s*:/i.test(prompt)
  };
}
...
function 恢复关键块(当前prompt, 原始prompt, maxLength = 1500) {
  ...
  for (const block of 原始关键块) {
    ...
    // v6.6.3-fix: 新增关键字段保护
    const isMood = block.includes('【情绪】') || /^MOOD\s*:/i.test(block);
    const isCamera = block.includes('【运镜】') || /^CAMERA\s*:/i.test(block);
    const isLighting = block.includes('【灯光】') || /^LIGHTING\s*:/i.test(block);
    const isRender = block.includes('【渲染参数】') || /^RENDER\s*:/i.test(block);
    const isDirector = block.includes('【导演指令】') || /^DIRECTOR\s*:/i.test(block);
    ...
    // v6.6.3-fix: 恢复新关键字段
    if (isMood && !当前状态.情绪 && (out.length + block.length + 3 <= maxLength)) {
      out += ` | ${block}`;
      当前状态.情绪 = true;
      continue;
    }
    if (isCamera && !当前状态.运镜 && (out.length + block.length + 3 <= maxLength)) {
      out += ` | ${block}`;
      当前状态.运镜 = true;
      continue;
    }
    if (isLighting && !当前状态.灯光 && (out.length + block.length + 3 <= maxLength)) {
      out += ` | ${block}`;
      当前状态.灯光 = true;
      continue;
    }
    if (isRender && !当前状态.渲染参数 && (out.length + block.length + 3 <= maxLength)) {
      out += ` | ${block}`;
      当前状态.渲染参数 = true;
      continue;
    }
    if (isDirector && !当前状态.导演指令 && (out.length + block.length + 3 <= maxLength)) {
      out += ` | ${block}`;
      当前状态.导演指令 = true;
      continue;
    }
...
function 最小补洞(prompt, shot = {}) {
  ...
  // v6.6.3-fix: 新增关键字段补洞
  const has情绪 = out.includes('【情绪】') || /MOOD\s*:/i.test(out);
  const has运镜 = out.includes('【运镜】') || /CAMERA\s*:/i.test(out);
  const has灯光 = out.includes('【灯光】') || /LIGHTING\s*:/i.test(out);
  const has渲染参数 = out.includes('【渲染参数】') || /RENDER\s*:/i.test(out);
  const has导演指令 = out.includes('【导演指令】') || /DIRECTOR\s*:/i.test(out);

  if (!has情绪 && (shot.emotionPhase || shot.mood)) {
    out += ` | 【情绪】${shot.emotionPhase || shot.mood}`;
  }
  if (!has运镜 && shot.cameraMovement) {
    const cm = shot.cameraMovement;
    const camStr = cm.string || (cm.description) || `${cm.shotSize || ''} ${cm.movement || ''}`.trim();
    if (camStr) out += ` | 【运镜】${camStr}`;
  }
  if (!has灯光 && shot.lighting) {
    const li = shot.lighting;
    const lightStr = li.string || (li.keyLight?.effect) || '';
    if (lightStr) out += ` | 【灯光】${lightStr}`;
  }
