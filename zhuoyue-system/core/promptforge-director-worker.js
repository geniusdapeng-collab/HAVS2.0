...
const LOCAL_MODE =
  process.env.PROMPTFORGE_LOCAL_MODE === 'true'
  || (
...
// v6.6.2-fix: 禁止静默降级环境开关
const DISABLE_FALLBACK = process.env.PROMPTFORGE_DISABLE_FALLBACK === 'true';
...
4. 对镜头创作的具体指导原则`;
    ...
    if (DISABLE_FALLBACK) {
      throw new Error(`Stage 1 禁止降级且LLM失败: ${e.message}`);
    }
...
3. 与画面的配合建议`;
    ...
    if (DISABLE_FALLBACK) {
      throw new Error(`Stage 2a 禁止降级且LLM失败: ${e.message}`);
    }
...
4. 构图要点（三分法、引导线、视觉焦点）`;
    ...
    if (DISABLE_FALLBACK) {
      throw new Error(`Stage 2b 禁止降级且LLM失败: ${e.message}`);
    }
...
请输出融合后的完整Prompt（仅输出Prompt内容，不要解释）：`;
    ...
    if (DISABLE_FALLBACK) {
      throw new Error(`Stage 3 禁止降级且LLM失败: ${e.message}`);
    }
