      ...
      try {
        ...
        for (const prompt of (result.stages.render || [])) {
          ...
          const shotData = {
            ...
            dialogue: prompt.prompt?.match(/【台词】([^【|]*)/)?.[1]?.trim() || '',
          ...
          if (finalShot && typeof finalShot === 'object') {
            const rebuildTags = [
              ...
              ['character', '角色'], ['action', '动作'], ['dialogue', '台词'],
...
const { spawn } = require('child_process');
  ...
  _validateTopicAdherence(scenes, input) {
    ...
    for (const scene of scenes) {
      const check = this._checkTopicDeviation(scene.dialogue, config);
...
${isNirath
  ? `{"scenes":[{"id":"S01","scene":"场景名称","dialogue":"角色对白或台词文本(不要旁白叙述,要角色自己说的话)","action":"角色具体动作描述,如双手展开、边走边转身","type":"opening","characters":["xiaoG","taotie"],"mouthAction":"speaking_whisper","emotionPhase":"curiosity","importance":8,"visualComplexity":7,"visualPrompt":"超写实,电影级光影,角色动作描述(300-500字)","beastDialogue":"异兽台词(如有,20字内)"}]}`
  : `{"scenes":[{"id":"S01","scene":"场景名称","dialogue":"角色对白或台词文本(不要旁白叙述,要角色自己说的话)","action":"角色具体动作描述,如双手展开、边走边转身","type":"explanation","characters":["${charList.split(',')[0] || 'chen-nurse'}"],"mouthAction":"speaking_normal","emotionPhase":"professional","importance":8,"visualComplexity":7,"visualPrompt":"超写实,电影级光影,角色动作描述(300-500字)"}]}`
}
...
- 场景名称是中文,台词内容也必须匹配中文场景名所暗示的主题`;
  ...
  async stageStoryboard(script, durations, input = {}) {
    ...
    for (let i = 0; i < mappedScenes.length; i++) {
      ...
      const shot = {
        ...
        dialogue: scene.dialogue || llmShot.dialogue || '',
  ...
  const { updateShotPromptIncrementally, appendJsonl, loadCheckpoint } = require('../../systems/pipeline-checkpoint');
    ...
    for (let i = 0; i < storyboard.shots.length; i++) {
      ...
      const lostFields = [];
      if (!stabilityCheck.台词 && (shot.dialogue || shot.narration)) lostFields.push('台词');
  ...
  toStandardPrompt(shot, prompt) {
    ...
    const dialogue = safe(shot.dialogue || shot.narration);
    if (!has.台词 && dialogue) {
      prefixPatches.push(`【台词】${dialogue}`);
    }
    if (!has.负面约束) {
  ...
  ensureFinalPromptStructure(shot, prompt) {
    ...
    if (!has.台词) {
      const dialogue = safe(shot.dialogue || shot.narration);
      if (dialogue) patches.push(`【台词】${dialogue}`);
    }
