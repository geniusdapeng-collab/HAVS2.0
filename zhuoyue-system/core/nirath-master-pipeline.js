      ...
      try {
        ...
        if (result.success && Array.isArray(result.data?.scenes)) {
          ...
          const normalized = batch.map((srcScene) => {
            ...
            // v6.6.5-fix: 场景名保护 - 强制使用配置表或原始名称
            const protectedName = config.sceneNameMapping[srcScene.id] || srcScene.name || generated.scene || '';
            return {
              ...srcScene,
              scene: protectedName,
...
- 场景名称是中文,台词内容也必须匹配中文场景名所暗示的主题`;
  ...
  toStandardPrompt(shot, prompt) {
    ...
    if (!has.场景 && shot.scene) {
      const scene = typeof shot.scene === 'string' ? shot.scene : (shot.scene?.name || '');
