 ...
 * 世界级IP短片生成引擎主链路入口
 *
 * v6.7.0: 25字段全链路升级 + 字段检查修复环节 + Agent体系 - 2026-06-23
 * - 25字段模板（P0-P3四级优先级）
 * - 3000字符上限 + 六步截断策略
 * - Agent体系：SceneDesignAgent, VisualLanguageAgent, AudioDesignAgent, DirectorSkillInjector
 * - FieldGuard + RedundancyDetector + CrossFieldConsistencyChecker
 * - FieldCheckAgent（规则+LLM双层检查）+ FieldRepairAgent（规则+LLM双通道修复）
 * - FieldQualityPipeline（maxRounds=2多轮迭代）
 * - Stage 11.7: 质量检查 + 冗余清理 + 一致性检查 + 导演技能增强
 * - Stage 11.8: 多轮字段质量迭代（PRD动态注入）
 * - 向后兼容：旧标签自动映射（绑定定妆照→定妆照等）
 *
 ...
 * - 违反 = 系统级错误,立即上报队长
 *
 * v6.6.4: 报告生成器升级 - 镜头内部详细时间轴 + 定妆照绝对路径绑定 - 2026-06-20
 * - generateReport: 添加镜头内部详细时间轴（按秒切分运镜/动作/画面）
 * - generateReport: 定妆照绑定使用绝对路径（characters/chenzhuo/portraits/...）
 * - generateReport: 添加附录：本集使用定妆照清单
 * - run-preproduction-v3.js: 同步升级报告生成逻辑
 * v6.6.3: SIGKILL 根因修复 —— 调用方 timeout 约束 + LLM 心跳保活 + 断点恢复 - 2026-06-20
 * - 真正根因: 调用方 exec timeout 过短（300s），完整预生产需 540-600s
 * - 要求: 调用方必须设置 timeout >= 900s（15分钟）
 * - LLM 调用期间 setInterval 心跳（5秒/次），提供进程存活观测
 * - 全局进程心跳兜底（run-preproduction-v3.js 顶部）
 * - Stage 5A/5B 真正的断点恢复（loadCheckpoint）
 * - PromptForge 子进程等待期父进程心跳 + stderr 透传
 * - LLM 超时 10分钟→2分钟
 * v6.6.2: 全局中文字段标准规范机制 + Prompt稳定性保护 + 增强模块配置开关 - 2026-06-18
 * - 新增 unified-shot-schema-zh.js: 中文字段标准（片头17字段/内容20字段）
 * - 新增 final-shot-standardizer.js: 标准化最终输出
 * - 新增 critical-field-gate.js: 关键字段硬闸门
 * - 新增 prompt-stability-guard.js: Prompt稳定性保护（只补不洗策略）
 * - 新增 field-loss-rootcause-logger.js: 字段丢失根因日志器
 * - 重写 toStandardPrompt: 只补不洗，保留原始主体
 * - 重写 ensureFinalPromptStructure: 极轻量补洞
 * - 修改 LOCAL_MODE: 根据API Key自动判断，不再硬编码
 * - 修改 stageSafetyGate: narration检查改为 dialogue || narration
 * - 增强模块配置开关: setDesign/motion/realism/closingBoost/narrativeArc/intraShot/lighting/audio
 * v6.6.1: Stage 11 事件循环让出 + Checkpoint 容错修复 - 2026-06-16
 * - Stage 11 每 shot 之间添加 setTimeout(0) 事件循环让出，防止进程被判定为僵死
 * - Stage 5A/11 checkpoint 写入添加 try-catch 保护，避免写入失败导致整个阶段崩溃
 * - 片头镜头完成后也落盘 checkpoint
 * - v6.6.1: 标题营销化设计修复 - 2026-06-16
 * - generic-opening-system.js: 主标题智能压缩 + 营销化增强
 ...
 * - 新增 runtime-memory-guard.js: 内存监控 + GC触发 + 强制清理
 * - 新增 pipeline-checkpoint.js: 断点落盘 + 增量写入
 * - LLMEngine 移除 raw result 大对象持有
 * - Stage 5A: 每批次前后 GC + 断点落盘
 * - Stage 11: 每 shot 前后 GC + 内存快照 + 增量JSONL
 * - 启动命令: NODE_OPTIONS="--max-old-space-size=3072 --expose-gc"
 * - v6.5.65-P8: 创意指数全局解析器 (Creativity Index Parser)
 * - 默认CP=0.2(不干预模式),用户明确输入时自动解析
 * - 支持自然语言输入:"非常有创意"→0.75,"创意天花板"→0.95,"极简"→0.1
 * - 指数翻译:模糊描述→数值→模块指令
 * - 集成到主链路需求解析环节,全局影响14个模块
 * - 向后兼容:支持数值、字符串、CLI参数
 * - v6.5.65-P7: 创意参数引擎 v2.0 (14模块细粒度控制)
 * - 14个模块:运镜/灯光/布景/剪辑/声音/色彩/构图/表演/特效/镜头语言/氛围/质感/时间/空间
 * - 每个模块5个等级:极简/标准/增强/艺术/极致
 * - 动态阈值:根据影片类型自动调整(科普片阈值更高)
 * - 安全边界:只影响外在表现,不修改内容(剧本/事实/医学数据/角色/对白)
 * - 与Stage 8.4联动:CP越高,注入更多电影级术语
 * - CLI支持:--creativity=0.9
 * - 融合设计:吸收对方14模块优点 + 161技能库联动 + 动态阈值 + 安全边界
 * - v6.5.65-P6: 创意参数引擎 (Creativity Parameter Engine)
 * - 支持 0.0-1.0 创意参数,5个等级:极简科普/标准纪录片/叙事增强/电影级/好莱坞大片
 * - 各阶段配置:PRD视觉风格、脚本语气、故事板镜头复杂度、运镜风格、技能注入强度、渲染质量、调色
 * - 安全边界:只影响外在表现,不修改内容(剧本/事实/医学数据/角色/对白)
 * - 与Stage 8.4联动:CP越高,注入更多电影级术语
 * - 向后兼容:默认CP=0.5(当前行为)
 * - v6.5.65-P5: 电影摄影技能库扩展 (161 skills, 15 film types)
 * - 技能库从9个扩展到161个,覆盖15种影片类型
 * - 支持剧情/动作/喜剧/恐怖/悬疑/惊悚/战争/科幻/孤独/微表情/纪录片/广告片/品牌片/科普片/通用
 * - 安全边界:只影响外在表现(影视风格/运镜/灯光/美术),不修改内容
 * - 动态扫描技能目录,自动索引所有技能文件
 * - 所有技能文件标注 scope: exterior_only
 * - v6.5.65-P4: 通用电影摄影技能注入(全类型)
 * - v6.5.13: generic模式修复
...
// ========== v6.6.2: 全局中文字段标准规范机制（外部专家审计修复）==========
const { 标准化最终输出 } = require('../../systems/final-shot-standardizer.js');
const { 检查关键字段 } = require('../../systems/critical-field-gate.js');

// ========== v6.6.2: Prompt 稳定性保护器（外部专家审计修复）==========
const {
  提取关键块,
  关键字段存在,
  稳定裁剪,
  恢复关键块,
  最小补洞
} = require('../../systems/prompt-stability-guard.js');

// ========== v6.7.0: 25字段全链路升级 - 新增Agent导入 ==========
const { SceneDesignAgent } = require('../../systems/agents/scene-design-agent.js');
const { VisualLanguageAgent } = require('../../systems/agents/visual-language-agent.js');
const { AudioDesignAgent } = require('../../systems/agents/audio-design-agent.js');
const { DirectorSkillInjector } = require('../../systems/agents/director-skill-injector.js');
const { FieldGuard } = require('../../systems/field-guard.js');
const { RedundancyDetector } = require('../../systems/field-guard/redundancy-detector.js');
const { CrossFieldConsistencyChecker } = require('../../systems/field-guard/cross-field-consistency.js');
const { FieldQualityPipeline } = require('../../systems/field-quality-pipeline.js');
...
// ========== v6.5.65-P6: 创意参数引擎 ==========
const { CreativityParameterEngine } = require('../../systems/creativity-parameter-engine-v2.js');
// ========== v6.5.65-P8: 创意指数解析器 ==========
const { CreativityIndexParser } = require('../../systems/creativity-index-parser.js');
...
const TOPIC_CONSISTENCY_CONFIG = {
  ...
  'health-edu-ep02-rhabdomyolysis-causes': {
    ...
    episodeTitle: '原因分析',
    topic: '横纹肌溶解的原因和机制',
    forbiddenContext: '症状表现、实验室检查、尿液颜色变化',
    requiredKeywords: [
      '运动过度', '剧烈运动', '肌肉损伤', '肌纤维',
      '他汀', '抗生素', '药物', '药物毒性',
      '饮酒', '酒精', '节食', '生酮', '饮食',
    ...
    forbiddenKeywords: [
      '症状', '肌肉疼痛', '肌肉无力', '肌无力', '肿胀', '压痛',
      '尿液颜色', '酱油色', '浓茶色', '茶色尿', '尿色', '尿',
      '肌酸激酶', '肌红蛋白', 'CK', 'MB', '肌酶',
      '实验室检查', '化验', '抽血', '检查指标'
    ],
    sceneNameMapping: {
  ...
  'health-edu-ep03-rhabdomyolysis-treatment': {
    seriesTitle: '健康科普系列：横纹肌溶解',
    episodeIndex: 3,
    episodeTitle: '治疗方案',
    topic: '横纹肌溶解的治疗方案',
    forbiddenContext: '症状表现、实验室检查、原因分析',
    requiredKeywords: ['补液', '利尿', '碱化尿液', '血液净化', '透析', '治疗', '用药', '停药', '康复'],
    forbiddenKeywords: ['症状', '原因', '机制', '诱因'],
    sceneNameMapping: {}
  },
  'health-edu-ep04-rhabdomyolysis-prevention': {
    seriesTitle: '健康科普系列：横纹肌溶解',
    episodeIndex: 4,
    episodeTitle: '预防措施',
    topic: '横纹肌溶解的预防',
    forbiddenContext: '症状表现、实验室检查、原因分析、治疗方案',
    requiredKeywords: ['预防', '避免', '循序渐进', '补水', '适度运动', '避免高温', '定期监测'],
    forbiddenKeywords: ['症状', '原因', '机制', '治疗', '透析'],
    sceneNameMapping: {}
  }
};
...
class NirathMasterPipeline {
  constructor(options = {}) {
    ...
    // v6.6.2: 增强模块配置开关（外部专家审计修复）
    this.enhancementConfig = {
      setDesign: options.enhancement?.setDesign !== false,      // 美术布景增强
      motion: options.enhancement?.motion !== false,             // 动作增强
      realism: options.enhancement?.realism !== false,           // 真实感增强
      closingBoost: options.enhancement?.closingBoost !== false, // 结尾情绪增强
      narrativeArc: options.enhancement?.narrativeArc !== false, // 叙事弧线注入
      intraShot: options.enhancement?.intraShot !== false,       // 镜头内增强
      lighting: options.enhancement?.lighting !== false,         // 灯光增强
      audio: options.enhancement?.audio !== false,               // 音频增强
    };
    this.log('INIT', `🎛️ 增强模块配置: ${JSON.stringify(this.enhancementConfig)}`);

    // ========== v6.7.0: 25字段全链路升级 - Agent与质量检查初始化 ==========
    this.fieldGuard = new FieldGuard({
      log: this.log.bind(this),
      maxChars: 3000,
      strictMode: true
    });
    this.redundancyDetector = new RedundancyDetector({ log: this.log.bind(this) });
    this.crossFieldChecker = new CrossFieldConsistencyChecker({ log: this.log.bind(this) });
    this.directorSkillInjector = new DirectorSkillInjector({ log: this.log.bind(this) });
    this.log('INIT', '🔍 v6.7.0 FieldGuard + RedundancyDetector + CrossFieldConsistency + DirectorSkillInjector 已初始化');

    // v6.7.0: 字段质量管线（检查+修复多轮迭代）
    this.fieldQualityPipeline = new FieldQualityPipeline({
      llm: this._createFieldQualityLLM(),
      log: this.log.bind(this),
      maxRounds: 2,
      prd: null // 将在运行时从stages.prd动态注入
    });
    this.log('INIT', '🔧 v6.7.0 FieldQualityPipeline 已初始化（maxRounds=2）');
  ...
  // ========== 主链路执行 ==========
  async execute(input, isResume = false) {
    const pipelineStart = Date.now(); // P1: 审计日志计时
    
    // v6.6.6-fix: 处理resume参数，将checkpoint数据注入input
    if (isResume && input?.__resumeStages) {
      this.log('PIPELINE', `🔄 恢复模式: 从断点继续 | 已完成阶段: ${Object.keys(input.__resumeStages).join(', ')}`);
    }
    
    // v6.6.6-fix-fix-fix: 保存input供下游stage访问(如stageCameraMovement的条件判断)
    // v6.6.6-fix: 类型归一化——EDU/educational/education/Educational 统一为 'educational'
    const normalizeType = (t) => {
      if (!t) return '';
      const lower = String(t).toLowerCase();
      if (lower === 'edu' || lower === 'educational' || lower === 'education') return 'educational';
      return lower;
    };
    if (input.videoType) input.videoType = normalizeType(input.videoType);
    if (input.filmType) input.filmType = normalizeType(input.filmType);
    this.isEducational = input.videoType === 'educational' || input.filmType === 'educational';
    this.isNirathContent = this.mode === 'nirath' && !this.isEducational;
    ...
    // ========== v6.5.65-P8: 创意指数解析与初始化 ==========
    // 1. 解析创意指数(支持自然语言)
    const creativityInput = input.creativityParameter || input.creativity || input.creativityIndex || input.CI;
    let cp;
    let cpSource;

    if (creativityInput !== undefined) {
      // 用户明确输入,使用解析器
      const parser = new CreativityIndexParser();
      const parsed = parser.parse(creativityInput);
      cp = parsed.value;
      cpSource = parsed.source;
      this.log('PIPELINE', `🎨 创意指数解析: "${creativityInput}" → CP=${cp} | 来源: ${cpSource}`);
    } else {
      // 默认0.2,不干预
      cp = 0.2;
      cpSource = 'default';
      this.log('PIPELINE', `🎨 创意指数: 默认 CP=${cp} (不干预模式)`);
    }

    const cpEngine = new CreativityParameterEngine(cp);
    const cpInfo = cpEngine.getLevelInfo();
    this.log('PIPELINE', `🎨 创意等级: ${cpInfo.level} | 风格: ${cpInfo.style || cpInfo.description}`);
    this.log('PIPELINE', `🎨 安全边界: 只影响外在表现(影视风格/运镜/灯光/美术),不修改内容`);

    // 将CP引擎附加到input,供各阶段使用
    input._cpEngine = cpEngine;
    input._cpInfo = cpInfo;
    ...
    try {
      ...
      // Stage 1: PRD中央校准文档生成
      // v6.5.65-P6: 注入创意参数配置
      const cpStage1Config = cpEngine.getStageConfig('1-prd');
      input._cpStage1Config = cpStage1Config;
      this.log('STAGE-1', `🎨 创意参数配置: 视觉风格=${cpStage1Config.visualStyle}, 叙事风格=${cpStage1Config.narrativeStyle}`);
      ...
      // Stage 5: 剧本生成与分析
      // v6.5.65-P6: 注入创意参数配置
      const cpStage5Config = cpEngine.getStageConfig('5-script');
      input._cpStage5Config = cpStage5Config;
      this.log('STAGE-5', `🎨 创意参数配置: 结构=${cpStage5Config.structure}, 语气=${cpStage5Config.tone}`);
      ...
      // v6.5.65-P8-fix: 将 S00 片头插入 storyboard.shots 首位
      if (result.stages.opening && result.stages.storyboard && result.stages.storyboard.shots) {
        const opening = result.stages.opening;
        const s00Shot = {
          id: 'S00',
          shotId: 'S00',
          type: 'opening',
          isOpening: true,
          scene: opening.scene || '片头-开场',
          duration: opening.duration || 8,
          prompt: opening.prompt || '',
          characters: opening.characters || [],
          mouthAction: '', // 片头无口播
          emotionPhase: opening.emotionPhase || 'curiosity',
          cameraMovement: opening.cameraMovement || { description: '平稳推进,展现标题与角色' },
          lighting: opening.lighting || { keyLight: '自然光,主标题明亮清晰' },
          narration: '', // 片头无旁白
          dialogue: '', // 片头无角色台词
          title: opening.title || { main: input.title || input.content?.title || '', sub: input.topic || input.content?.topic || '' }
        };
        result.stages.storyboard.shots.unshift(s00Shot);
        this.log('PIPELINE', `🎬 S00 片头已插入 storyboard.shots 首位 | 时长: ${s00Shot.duration}秒 | 标题: ${s00Shot.title?.main || '未命名'}`);
      }

      // v6.6.6-fix: 去重 shot.scene，确保相邻镜头场景描述不同
      const seenScenes = new Set();
      for (const shot of result.stages.storyboard.shots || []) {
        if (shot.scene && seenScenes.has(shot.scene)) {
          const originalScene = shot.scene;
          shot.scene = `${originalScene}（${shot.id}）`;
          this.log('PIPELINE', `🔄 场景去重: ${shot.id} | ${originalScene} -> ${shot.scene}`);
        }
        seenScenes.add(shot.scene);
      }
      ...
      // v6.5.65-P6: 注入创意参数配置
      const cpStage9Config = cpEngine.getStageConfig('9-camera');
      input._cpStage9Config = cpStage9Config;
      this.log('STAGE-9', `🎨 创意参数配置: 运镜风格=${cpStage9Config?.movementStyle || 'default'}, 运动强度=${cpStage9Config?.movementIntensity || 'default'}`);
      ...
      // 🔥 v6.5.65-P4: Stage 8.4 电影摄影技能注入(通用,全类型)
      // 在运镜系统完成后,增强prompt的电影级质量
      // 安全边界:只影响外在表现(影视风格/运镜/灯光/美术),不修改内容(剧本/事实/角色/对白)
      // v6.5.65-P6: 创意参数控制技能注入强度
      const skillInjectionParams = cpEngine.getSkillInjectionParams();
      this.log('STAGE-8.4', `🎨 创意参数配置: 技能注入=${skillInjectionParams.enabled ? '启用' : '禁用'} | 最大术语=${skillInjectionParams.maxTerms} | 强度=${skillInjectionParams.aggressiveness}`);

      result.stages.filmCinematographySkills = await runStage('STAGE-8.4', async () => {
        try {
          const { stage84FilmCinematographyInjection, isSkillLibraryAvailable } = require('./stage84-film-cinematography-injection');

          if (!skillInjectionParams.enabled) {
            this.log('STAGE-8.4', '⏭️ 创意参数低于阈值,技能注入禁用');
            return { applied: false, reason: 'cp_below_threshold', cp: cp };
          }

          if (!isSkillLibraryAvailable()) {
            this.log('STAGE-8.4', '⏭️ 技能库不可用,跳过');
            return { applied: false, reason: 'skill_library_unavailable' };
          }

          // 将技能注入参数传递给Stage 8.4
          input._skillInjectionParams = skillInjectionParams;
          const enhanced = await stage84FilmCinematographyInjection(this, result.stages.storyboard, input);

          // 更新故事板(增强后的prompt已写入stages.style.prompt)
          if (enhanced && enhanced._stage84Applied) {
            result.stages.storyboard = enhanced;
            return {
              applied: true,
              enhancedShots: enhanced.shots?.length || 0,
              skillsUsed: enhanced._filmicSkillsUsed || [],
              cp: cp,
              maxTerms: skillInjectionParams.maxTerms
            };
          }

          return { applied: false, reason: 'no_enhancement_needed' };
        } catch (err) {
          this.log('STAGE-8.4', `⚠️ 技能注入失败: ${err.message} | 继续生产流程`);
      ...
      // v6.5.65-P6: 注入创意参数配置(渲染质量、灯光复杂度、调色)
      const cpStage11Config = cpEngine.getStageConfig('11-render');
      input._cpStage11Config = cpStage11Config;
      this.log('STAGE-11', `🎨 创意参数配置: 渲染质量=${cpStage11Config.promptQuality}, 灯光=${cpStage11Config.lightingComplexity}, 调色=${cpStage11Config.colorGrading}`);
      ...
      // ========== v6.7.0: FieldGuard 25字段质量检查 ==========
      try {
        this.log('STAGE-11.7', '🔍 启动FieldGuard 25字段质量检查...');
        const fgResults = [];
        for (const prompt of (result.stages.render || [])) {
          const shot = result.stages.storyboard?.shots?.find(s => s.id === prompt.shotId) || {};
          const fgResult = await this.fieldGuard.validate(prompt.prompt || prompt, shot);
          fgResults.push({ shotId: prompt.shotId, ...fgResult });
        }
        const failedShots = fgResults.filter(r => !r.passed);
        if (failedShots.length > 0) {
          this.log('STAGE-11.7', `⚠️ FieldGuard发现${failedShots.length}个镜头未通过P0检查:`);
          for (const f of failedShots) {
            this.log('STAGE-11.7', `  ❌ ${f.shotId}: ${f.errors.slice(0, 2).join('; ')}`);
          }
          // v6.7.0-patch: FieldGuard P0 未通过的镜头，强制走 toStandardPrompt 二次补洞
          this.log('STAGE-11.7', `🔧 对 ${failedShots.length} 个未通过镜头执行强制补洞...`);
          for (const f of failedShots) {
            const p = (result.stages.render || []).find(r => r.shotId === f.shotId);
            const s = result.stages.storyboard?.shots?.find(x => x.id === f.shotId) || {};
            if (p) {
              p.prompt = this.toStandardPrompt(s, p.prompt || '');
              this.log('STAGE-11.7', `  🔄 ${f.shotId}: 强制补洞完成 | ${p.prompt.length}字符`);
            }
          }
          // 重新校验
          ...
          if (stillFailed.length > 0) {
            ...
            // 可选：写入 result 供审片环节人工介入
            result.stages.fieldGuard.blockedShots = stillFailed.map(s => s.shotId);
          }
        } else {
          this.log('STAGE-11.7', `✅ FieldGuard通过 | ${fgResults.length}个镜头全部合格`);
        }

        // 冗余检测
        let totalSaved = 0;
        for (const prompt of (result.stages.render || [])) {
          const redundancies = this.redundancyDetector.detect(prompt.prompt || prompt);
          if (redundancies.length > 0) {
            const { prompt: cleaned, savedChars } = this.redundancyDetector.remove(prompt.prompt || prompt, redundancies);
            if (savedChars > 0) {
              prompt.prompt = cleaned;
              totalSaved += savedChars;
            }
          }
        }
        if (totalSaved > 0) {
          this.log('STAGE-11.7', `🧹 冗余清理完成 | 共释放${totalSaved}字符`);
        }

        // 跨字段一致性检查
        const consistencyIssues = [];
        for (const prompt of (result.stages.render || [])) {
          const issues = this.crossFieldChecker.check(prompt.prompt || prompt);
          if (issues.length > 0) {
            consistencyIssues.push({ shotId: prompt.shotId, issues });
          }
        }
        if (consistencyIssues.length > 0) {
          this.log('STAGE-11.7', `⚠️ 跨字段一致性检查发现${consistencyIssues.length}个镜头存在问题:`);
          for (const c of consistencyIssues) {
            const critical = c.issues.filter(i => i.severity === 'error');
            if (critical.length > 0) {
              this.log('STAGE-11.7', `  ❌ ${c.shotId}: ${critical[0].description}`);
            }
          }
        }

        // ========== v6.7.0: DirectorSkillInjector 导演技能增强 ==========
        try {
          let enhancedCount = 0;
          for (const prompt of (result.stages.render || [])) {
            const shot = result.stages.storyboard?.shots?.find(s => s.id === prompt.shotId) || {};
            const context = {
              mood: shot.emotionPhase || shot.mood || 'neutral',
              sceneType: typeof shot.scene === 'string' ? 'interior' : (shot.scene?.type || 'interior'),
              sceneScale: shot.sceneScale || 'medium'
            };
            const enhanced = await this.directorSkillInjector.enhance(prompt.prompt || prompt, context);
            if (enhanced !== (prompt.prompt || prompt)) {
              prompt.prompt = enhanced;
              enhancedCount++;
            }
          }
          if (enhancedCount > 0) {
            this.log('STAGE-11.7', `🎬 导演技能增强完成 | ${enhancedCount}个镜头已增强`);
          }
        } catch (e) {
          this.log('STAGE-11.7', `⚠️ 导演技能增强跳过: ${e.message}`);
        }

        result.stages.fieldGuard = { passed: failedShots.length === 0, results: fgResults, consistencyIssues };
      } catch (e) {
        this.log('STAGE-11.7', `❌ FieldGuard检查异常: ${e.message}`);
        // v6.7.0-patch: 检查异常不能标记为通过，应标记为失败供下游阻断
        result.stages.fieldGuard = { passed: false, skipped: true, error: e.message, fatal: true };
      }

      // ========== v6.7.0: Stage 11.8 FieldQualityPipeline 字段质量检查+修复 ==========
      try {
        this.log('STAGE-11.8', '🔧 启动FieldQualityPipeline（检查+修复多轮迭代）...');
        
        // 构建PRD数据
        const prdData = {
          project_name: this.projectConfig?.name || result.stages.prd?.projectName || '',
          video_type: this.projectConfig?.filmType || 'generic',
          style_direction: result.stages.prd?.visualStyle || '',
          mood_tone: result.stages.prd?.moodTone || '',
          characters: result.stages.prd?.characters || [],
          scenes: result.stages.prd?.scenes || [],
          dialogues: result.stages.prd?.dialogues || [],
          special_constraints: result.stages.prd?.specialConstraints || [],
          target_platform: result.stages.prd?.targetPlatform || ''
        };
        
        // 更新FieldQualityPipeline的PRD（v6.7.0-patch: 同时注入到 repairer，否则 LLM 修复永不触发）
        this.fieldQualityPipeline.prd = prdData;
        if (this.fieldQualityPipeline.repairer) {
          const { PRD } = require('../../systems/field-repair/field-repair-agent');
          this.fieldQualityPipeline.repairer.prd = new PRD(prdData);
        }
        ...
        for (const prompt of (result.stages.render || [])) {
          ...
          // 将shot转换为25字段格式
          const shotData = {
            director_instruction: prompt.prompt?.match(/【导演指令】([^【|]*)/)?.[1]?.trim() || '',
            constraint: prompt.prompt?.match(/【约束】([^【|]*)/)?.[1]?.trim() || '',
            baseline: prompt.prompt?.match(/【基础】([^【|]*)/)?.[1]?.trim() || '',
            scene: prompt.prompt?.match(/【场景】([^【|]*)/)?.[1]?.trim() || '',
            lighting: prompt.prompt?.match(/【灯光[^】]*】([^【|]*)/)?.[1]?.trim() || '',
            camera_movement: prompt.prompt?.match(/【运镜】([^【|]*)/)?.[1]?.trim() || '',
            character: prompt.prompt?.match(/【角色】([^【|]*)/)?.[1]?.trim() || '',
            action: prompt.prompt?.match(/【动作】([^【|]*)/)?.[1]?.trim() || '',
            dialogue: prompt.prompt?.match(/【对话指令】([^【|]*)/)?.[1]?.trim() || prompt.prompt?.match(/【台词】([^【|]*)/)?.[1]?.trim() || '',
            negative: prompt.prompt?.match(/【负面约束】([^【|]*)/)?.[1]?.trim() || '',
            portraits: prompt.prompt?.match(/【定妆照】([^【|]*)/)?.[1]?.trim() || '',
            consistency: prompt.prompt?.match(/【角色一致性】([^【|]*)/)?.[1]?.trim() || '',
            composition: prompt.prompt?.match(/【构图】([^【|]*)/)?.[1]?.trim() || '',
            color_palette: prompt.prompt?.match(/【色彩[^】]*】([^【|]*)/)?.[1]?.trim() || '',
            depth_of_field: prompt.prompt?.match(/【景深】([^【|]*)/)?.[1]?.trim() || '',
            timeline: prompt.prompt?.match(/【时间轴】([^【|]*)/)?.[1]?.trim() || '',
            mood: prompt.prompt?.match(/【情绪】([^【|]*)/)?.[1]?.trim() || '',
            bright_constraint: prompt.prompt?.match(/【明亮约束】([^【|]*)/)?.[1]?.trim() || '',
            character_constraint: prompt.prompt?.match(/【角色约束】([^【|]*)/)?.[1]?.trim() || '',
            costume: prompt.prompt?.match(/【服装】([^【|]*)/)?.[1]?.trim() || '',
            props: prompt.prompt?.match(/【道具】([^【|]*)/)?.[1]?.trim() || '',
            pacing: prompt.prompt?.match(/【节奏】([^【|]*)/)?.[1]?.trim() || '',
            audio: prompt.prompt?.match(/【音频】([^【|]*)/)?.[1]?.trim() || '',
            makeup: prompt.prompt?.match(/【化妆】([^【|]*)/)?.[1]?.trim() || '',
            transition: prompt.prompt?.match(/【转场】([^【|]*)/)?.[1]?.trim() || '',
          };
          
          const { finalShot, reports, logs } = await this.fieldQualityPipeline.run(shotData, shot.id || prompt.shotId);

          // v6.7.0-patch: 将修复后的 finalShot 写回 prompt（原代码遗漏，导致修复无效）
          if (finalShot && typeof finalShot === 'object') {
            const rebuildTags = [
              ['director_instruction', '导演指令'], ['constraint', '约束'], ['baseline', '基础'],
              ['scene', '场景'], ['lighting', '灯光'], ['camera_movement', '运镜'],
              ['character', '角色'], ['action', '动作'], ['dialogue', '台词'], ['dialogueBlock', '对话指令'],
              ['negative', '负面约束'], ['portraits', '定妆照'], ['consistency', '角色一致性'],
              ['composition', '构图'], ['color_palette', '色彩'], ['depth_of_field', '景深'],
              ['timeline', '时间轴'], ['mood', '情绪'], ['bright_constraint', '明亮约束'],
              ['character_constraint', '角色约束'], ['costume', '服装'], ['props', '道具'],
              ['pacing', '节奏'], ['audio', '音频'], ['makeup', '化妆'], ['transition', '转场'],
            ];
            const parts = [];
            for (const [en, cn] of rebuildTags) {
              const val = (finalShot[en] || '').toString().trim();
              if (val) parts.push(`【${cn}】${val}`);
            }
            if (parts.length > 0) {
              // 保留原 prompt 中已有的非25字段内容（如 @image 引用、嘴部动作等）
              const extras = (prompt.prompt || '')
                .split(/\s*\|\s*|\s+/)
                .filter(seg => seg && !/^【/.test(seg) && !rebuildTags.some(([_, cn]) => seg.includes(`【${cn}】`)));
              prompt.prompt = parts.join(' | ') + (extras.length ? ' | ' + extras.join(' ') : '');
              // 重新走 smartTrim 防超长
              prompt.prompt = this.smartTrim ? this.smartTrim(prompt.prompt, 3000) : prompt.prompt.slice(0, 3000);
              this.log('STAGE-11.8', `  🔄 ${shot.id || prompt.shotId}: 修复结果写回 prompt | ${parts.length}字段 | ${prompt.prompt.length}字符`);
            }
          }

          // 如果修复后有变化，更新prompt
          if (reports.length > 0 && !reports[reports.length - 1].passed) {
            this.log('STAGE-11.8', `  ⚠️ ${shot.id || prompt.shotId}: 经${reports.length}轮检查，仍有${reports[reports.length - 1].issues.length}个问题未修复`);
          } else if (logs.length > 0) {
            this.log('STAGE-11.8', `  ✅ ${shot.id || prompt.shotId}: 经${logs.length}轮修复，检查通过`);
          }
          
          pipelineResults.push({
            shotId: shot.id || prompt.shotId,
            finalShot,
            reports,
            logs
          });
        ...
        result.stages.fieldQualityPipeline = {
          passed: pipelineResults.every(r => r.reports.length > 0 && r.reports[r.reports.length - 1].passed),
          results: pipelineResults
        };
      } catch (e) {
        this.log('STAGE-11.8', `❌ FieldQualityPipeline异常: ${e.message}`);
        // v6.7.0-patch: 修复管线异常不能标记为通过
        result.stages.fieldQualityPipeline = { passed: false, skipped: true, error: e.message, fatal: true };
      }
...
const { spawn } = require('child_process');
          ...
          // 严格内存限制:5GB(系统6GB,留1GB给主进程和系统)
          // v6.7.0-fix: 从4GB增加到5GB，因为25字段+3000字符导致上下文增大
          ...
          // v6.6.3-fix: stderr 改为 inherit 后 child.stderr 为 null
          // 因此需要条件监听；stderr 已直接透传至父进程终端，无需程序收集
          if (child.stderr) {
          ...
          // v6.6.3-fix: 父进程等待期间心跳保活
          // 问题: 等待子进程 close 期间父进程 stdout 静默，子代理 watchdog 可能误杀父进程
          await new Promise((resolve, reject) => {
            ...
            // v6.6.3-fix: 父进程心跳 —— 等待子进程期间每 5 秒输出心跳
            let pfTicks = 0;
            const pfHeartbeat = setInterval(() => {
              pfTicks++;
              this.log('PIPELINE', `⏳ PromptForge 子进程运行中... ${pfTicks * 5}s`);
            }, 5000);

            // v6.7.0-fix: 增强子进程退出处理，区分正常退出、SIGKILL、SIGTERM
            child.on('close', (code, signal) => {
              clearTimeout(timeout);
              clearInterval(pfHeartbeat);
              
              if (code === 0) {
                resolve();
              } else if (code === null && signal === 'SIGKILL') {
                // OOM 被系统杀死
                reject(new Error(`PromptForge 子进程被 OOM 杀死 (SIGKILL)。建议：1) 减少镜头数 2) 降低创意指数 3) 联系管理员增加内存`));
              } else if (code === 137) {
                // 128 + 9 = 137 (SIGKILL)
                reject(new Error(`PromptForge 子进程被 OOM 杀死 (退出码 137)。建议：1) 减少镜头数 2) 降低创意指数 3) 联系管理员增加内存`));
              } else if (signal) {
                reject(new Error(`PromptForge 子进程被 ${signal} 信号终止`));
              } else {
                reject(new Error(`PromptForge 子进程退出码 ${code}`));
              }
            });

            // v6.6.3-fix: 子进程异常退出也要清心跳
            child.on('error', (err) => {
              clearTimeout(timeout);
              clearInterval(pfHeartbeat);
              reject(new Error(`PromptForge 子进程启动失败: ${err.message}`));
            });
          });
      ...
      // Stage 14: 风格注入
      // v6.5.65-P6: 注入创意参数配置(色调风格、质感)
      const cpStage14Config = cpEngine.getStageConfig('14-style');
      input._cpStage14Config = cpStage14Config;
      this.log('STAGE-14', `🎨 创意参数配置: 色调=${cpStage14Config.toneStyle}, 质感=${cpStage14Config.textureQuality}`);
      ...
      // v6.6.6-fix: 遇到严重错误时提示联系原作者
      try {
        const { formatErrorWithContact } = require('../../author-info.js');
        this.log('PIPELINE', formatErrorWithContact(''));
      } catch (e) { /* 作者信息模块可选 */ }
  ...
  result.performance = {
    ...
    // v6.5.65-P8: 添加创意指数报告
    result.creativityParameter = cp;
    result.creativityLevel = cpInfo.level;
    result.creativityStyle = cpInfo.style || cpInfo.description;
    result.creativitySource = cpSource; // 解析来源
    result.creativityActiveModules = cpEngine.getActiveModules().activeModules; // 激活模块数
    result.creativitySafety = cpEngine.getSafetyReport(); // 安全边界报告
  ...
  async stagePRD(input) {
    this.log('STAGE-1', 'PRD生成 (统一数据结构版)');
    ...
    // 统一数据结构:确保所有必需字段存在
    prd.title = prd.title || input.projectName || '未命名项目';
    prd.topic = prd.topic || prd.core?.theme || input.core?.theme || prd.title;
    prd.targetDuration = prd.targetDuration || input.targetDuration || 60;

    // 确保keyPoints存在(Schema必需)
    if (!prd.keyPoints || !Array.isArray(prd.keyPoints) || prd.keyPoints.length === 0) {
      prd.keyPoints = [];
      if (prd.topic) prd.keyPoints.push(prd.topic);
      if (prd.core?.narrativeFocus) prd.keyPoints.push(prd.core.narrativeFocus);
      if (prd.core?.theme) prd.keyPoints.push(prd.core.theme);
      if (prd.keyPoints.length === 0) prd.keyPoints.push('核心内容');
    }
    ...
    prd.meta = prd.meta || {
      ...
      title: prd.title || input.projectName || '未命名项目' // v6.5.65-P8-fix: 添加 meta.title
    };

    // 确保 world/style/constraints 存在(向后兼容)
    prd.world = prd.world || input.world || {};
    // 确保 style 是对象(兼容字符串输入)
    if (typeof prd.style === 'string') {
      prd.style = { visualStyle: prd.style, colorPalette: '', pacing: '', mood: '', reference: '' };
    } else if (!prd.style || typeof prd.style !== 'object') {
      prd.style = {};
    }
    // 安全合并:仅当 input.style 是对象时才合并
    if (input.style && typeof input.style === 'object' && !Array.isArray(input.style)) {
      ...
      // 字符串类型的 input.style 映射到 visualStyle
      prd.style.visualStyle = input.style;
    }
    ...
    this.log('STAGE-1', `✅ PRD完成 | 场景: ${prd.scenes.length} | 角色: ${Object.keys(prd.characters).length} | 标题: ${prd.title} | 要点: ${prd.keyPoints.length}个`);
    return prd;
  }

  /**
   * v6.5.64-P0: PRD结构化fallback (统一数据结构版)
   * 确保输出符合统一视频需求结构,兼容Schema校验
   */
  _buildFallbackPRD(input) {
    ...
    // 提取核心信息,确保顶层字段存在
    const title = input.projectName || input.title || input.content?.title || '未命名项目';
    const topic = input.topic || input.content?.topic || input.core?.theme || input.core?.narrative?.focus || title;
    const targetDuration = input.targetDuration || 60;

    // 构建关键要点数组
    const keyPoints = [];
    if (input.core?.theme) keyPoints.push(input.core.theme);
    if (input.core?.narrative?.focus) keyPoints.push(input.core.narrative.focus);
    if (input.keyPoints) keyPoints.push(...input.keyPoints);
    if (input.content?.keyPoints) keyPoints.push(...input.content.keyPoints);
    if (keyPoints.length === 0) keyPoints.push(topic);

    return {
      // 一、视频任务基本信息
      title: title,
      topic: topic,
      videoType: this.input?.videoType || this.input?.projectType || 'EDU',
      targetAudience: input.targetAudience || input.content?.targetAudience || '普通大众',
      platform: input.platform || '视频号/抖音',

      // 二、制作规格
      targetDuration: targetDuration,
      aspectRatio: input.aspectRatio || '9:16',
      visualStyle: input.visualStyle || 'REAL+WARM',
      qualityLevel: input.qualityLevel || '电影级',
      colorTone: input.colorTone || '自然暖色调',

      // 三、内容创意要求
      creativityIndex: input.creativityIndex || input.creativity || input.creativityParameter || 0.2,
      narrativeStyle: input.narrativeStyle || '单人讲解',
      contentStyle: input.contentStyle || '专业严谨+通俗易懂',
      visualStyleDetail: input.visualStyleDetail || '写实风格,人物与背景均要求真实质感',
      musicStyle: input.musicStyle || '轻柔背景音乐+清晰人声',
      ...
      episodeThemes: input.episodeThemes || [],
      contentIsolation: input.contentIsolation || '',
      ...
      // 九、风格与约束
      // v6.5.65-P8-fix: 确保 style 是对象
      style: (typeof input.style === 'string')
        ? { visualStyle: input.style, colorPalette: '自然暖色', pacing: '适中', mood: '专业亲和', reference: '' }
        : (input.style || {
            visualStyle: '写实',
            colorPalette: '自然暖色',
            pacing: '适中',
      ...
      meta: {
        ...
        mode: this.mode,
        createdAt: new Date().toISOString(),
        aiReasoning: 'Fallback PRD: 基于输入自动填充缺失字段'
      }
    };
  }
    ...
    try {
      const { result, driver, attempts } = await this.llmEnforcer.requireLLM(
        ...
        () => StagePrompts.STAGE_2_ALIGNMENT(input, prd),
        {
          ...
          llmOptions: { maxTokens: 2048, temperature: 1 } // v6.6.6-fix-fix: kimi-k2p6 requires temperature=1
        }
      );
  ...
  async stageSchemaValidation(prd) {
    ...
    // v6.5.65-P8-fix: 强制归一化 style 字段,确保为对象
    if (typeof prd.style === 'string') {
      this.log('STAGE-3', `🔧 style字段归一化: string → object (${prd.style})`);
      prd.style = { visualStyle: prd.style, colorPalette: '', pacing: '', mood: '', reference: '' };
    } else if (!prd.style || typeof prd.style !== 'object' || Array.isArray(prd.style)) {
      prd.style = { visualStyle: '', colorPalette: '', pacing: '', mood: '', reference: '' };
    }
  ...
  async stageCharacters(input, prd) {
    ...
    for (const charId of characterIds) {
      ...
      if (this.modules.characterManager.characterExists(charId)) {
        ...
        // v6.6.3-fix: 如果加载的角色卡缺少portraitSets(可能是旧格式或被覆盖的空卡),尝试备选ID
        if (!charProfile || !charProfile.portraitSets) {
          const altCharId = charId.replace(/-/g, '');
          if (altCharId !== charId && this.modules.characterManager.characterExists(altCharId)) {
            const altProfile = await this.modules.characterManager.loadCharacter(altCharId);
            if (altProfile && altProfile.portraitSets) {
              charProfile = { ...altProfile, id: charId };
        ...
        // v6.6.3-fix: 尝试用备选ID(去掉连字符)查找原始角色卡
        const altCharId = charId.replace(/-/g, '');
        let altProfile = null;
        if (altCharId !== charId && this.modules.characterManager.characterExists(altCharId)) {
          altProfile = await this.modules.characterManager.loadCharacter(altCharId);
        }
        if (altProfile && altProfile.portraitSets) {
          // 使用原始角色卡的数据,但保留当前charId
          charProfile = { ...altProfile, id: charId };
          // 保存到当前charId目录,方便下次直接使用
          await this.modules.characterManager.saveCharacter(charId, charProfile);
        } else {
          charProfile = this.modules.characterManager.createCharacter(charId, fullCharData);
        }
      ...
      const portraitSets = charProfile?.portraitSets || {};
      for (const [setName, setData] of Object.entries(portraitSets)) {
        const setPortraits = setData?.portraits || [];
        for (const p of setPortraits) {
          if (p.angle && p.file) {
            // 使用 setName 作为前缀区分不同风格
            const key = setName === 'uniform' ? p.angle : `${setName}_${p.angle}`;
            portraits[key] = p.file;
            // 同时保留无前缀版本，方便下游选择
      ...
      // 如果 portraitSets 没有数据，fallback 到旧格式
      ...
      // 如果还是没有，从 portraitConfig 推断路径（旧兼容）
      if (Object.keys(portraits).length === 0) {
  ...
  async stageScriptGeneration(input, prd) {
    ...
    // v6.5.65-P8-fix: 全局禁用旁白,Stage 5 完成后强制清除所有 narration 字段
    if (script.scenes) {
      for (const scene of script.scenes) {
        if (scene.narration && scene.narration !== scene.dialogue) {
          this.log('STAGE-5', `  📝 ${scene.id}: 清除 narration (${scene.narration.length}字), 保留 dialogue`);
        }
        scene.narration = ''; // 强制清空
      }
    }
  ...
  _fallbackScript(input) {
    const analyzedScenes = (input.scenes || []).map((scene, idx) => {
      ...
      return {
        ...
        // v6.7.0-dialogue-patch: 支持 dialogueBlock 结构化对话
        dialogueBlock: scene.dialogueBlock || null,
        // v6.5.65-P8-fix: 全局禁用旁白,只保留角色对话(dialogue),不使用 narration 作为 fallback
        dialogue: scene.dialogue || (scene.dialogueBlock?.text || ''),
        narration: '', // 旁白已禁用,置空
        action: scene.action || '', // v6.5.65-P8-patch-008: 保留动作描述
  ...
  async _llmGenerateScript(input, prd) {
    ...
    // Phase A: 生成剧本骨架(轻量)
    let phaseAScenes = await this._generateScriptCorePhase(input);

    // v6.6.5-fix: 主题一致性校验 + 自动修正
    const { LLMEngine } = require('../../systems/llm-reasoning-engine');
    const correctionLlm = new LLMEngine({
      model: 'kimi-k2p6',
      mode: 'production',
      maxRetries: 2,
      maxTokens: 2048,
      temperature: 1, // v6.6.6-fix-fix: kimi-k2p6 requires temperature=1
      topP: 0.95, // v6.6.6-fix-fix: kimi-k2p6 requires top_p=0.95
      timeoutMs: 60000
    });

    let validation = this._validateTopicAdherence(phaseAScenes, input);
    if (!validation.passed) {
      this.log('STAGE-5', `🚨 Phase A 主题校验未通过，平均偏离度: ${validation.avgDeviation.toFixed(2)}，失败场景: ${validation.failedScenes}`);
      phaseAScenes = await this._autoCorrectScenes(phaseAScenes, validation, correctionLlm);
      const revalidation = this._validateTopicAdherence(phaseAScenes, input);
      if (revalidation.passed) {
        this.log('STAGE-5', `✅ Phase A 自动修正后通过`);
      } else {
        this.log('STAGE-5', `⚠️ Phase A 自动修正后仍未通过，偏离度: ${revalidation.avgDeviation.toFixed(2)}`);
        await this._logTopicDeviation(phaseAScenes, revalidation, input);
      }
    } else {
      this.log('STAGE-5', `✅ Phase A 主题校验通过`);
    }
  ...
  async _generateScriptCorePhase(input) {
    const { LLMEngine } = require('../../systems/llm-reasoning-engine');
    const { memoryReliefPoint, snapshotMemory } = require('../../systems/runtime-memory-guard');
    // v6.6.3-fix: 导入 loadCheckpoint 用于断点恢复
    const { appendJsonl, saveCheckpoint, loadCheckpoint } = require('../../systems/pipeline-checkpoint');
    ...
    const llm = new LLMEngine({
      model: 'kimi-k2p6',
      mode: 'production',
      maxRetries: 2, // v6.6.5-fix: 降低为 2，避免无效 200s 重试
      maxTokens: 3072,
      temperature: 1, // v6.6.6-fix-fix: kimi-k2p6 requires temperature=1
      topP: 0.95, // v6.6.6-fix-fix: kimi-k2p6 requires top_p=0.95
      timeoutMs: 120000 // v6.6.3-fix: 缩短超时
    });

    const scenes = input.scenes || [];
    const core = input.characters || {};
    const isNirath = this.isNirathContent;
    const world = {
      name: isNirath ? 'Nirath' : (this.input?.world?.name || this.input?.projectName || '现实世界'),
      setting: isNirath ? '外星生态星球' : (this.input?.world?.setting || this.input?.style || '超写实纪录片风格')
    };
    ...
    // v6.6.3-fix: 断点恢复 —— 加载已完成的批次，跳过重复工作
    // 之前只 save 不 load，即使被杀重启也会从第 0 批重跑，浪费已完成工作
    let startBatchIdx = 0;
    const saved = loadCheckpoint(stage5ACheckpoint, null);
    if (saved && Array.isArray(saved.results) && (saved.batchCount > 0 || saved.completedBatches > 0)) {
      const completed = saved.batchCount || saved.completedBatches;
      if (completed < batches.length) {
        this.log('STAGE-5A', `🔄 发现断点: 已完成 ${completed}/${batches.length} 批次，从批次 ${completed + 1} 恢复`);
        results.push(...saved.results);
        startBatchIdx = completed;
      } else if (completed >= batches.length) {
        this.log('STAGE-5A', `✅ 断点显示全部 ${completed} 批次已完成，跳过 Stage 5A`);
        await memoryReliefPoint('stage5A:end');
    ...
    for (let batchIdx = startBatchIdx; batchIdx < batches.length; batchIdx++) {
      ...
      const prompt = this._buildScriptCorePrompt(batch, core, world, input, batchIdx, batches.length);

      // v6.6.3-fix: 事件循环让出，防止进程被判定为僵死（与 Stage 11 保持一致）
      if (batchIdx > startBatchIdx) await new Promise(r => setTimeout(r, 0));
      ...
      const schema = {
        scenes: batch.map((scene) => ({
          ...
          dialogue: '',
          narration: '',
          action: '', // v6.5.65-P8-patch-008: 角色动作描述
          characters: scene.characters || [],
      ...
      let result = null;
      try {
        result = await llm.reasonStructured(prompt, schema, {
          maxTokens: 3072,
          temperature: 1, // v6.6.6-fix-fix: kimi-k2p6 requires temperature=1
        });

        if (result.success && Array.isArray(result.data?.scenes)) {
          // v6.6.5-fix: 场景名保护 + 内容清洗
          const config = this._getTopicConfig(input.projectName, input.topic);
          const rawData = this._extractAndSanitizeContent(result, config);
          const scenesData = rawData?.scenes || result.data.scenes;

          const normalized = batch.map((srcScene) => {
            const generated = scenesData.find((x) => x.id === srcScene.id) || {};
            ...
            // v6.7.0-dialogue-patch: 生成 fallback dialogueBlock
            const fallbackText = generated.dialogue || this._buildFallbackDialogue(srcScene, input.characters);
            const fallbackBlock = this._buildFallbackDialogueBlock(srcScene, input.characters, fallbackText);
            return {
              ...srcScene,
              scene: protectedName,
              // v6.7.0-dialogue-patch: 支持 dialogueBlock 结构化对话
              dialogueBlock: generated.dialogueBlock || fallbackBlock,
              // v6.5.65-P8-fix: 全局禁用旁白,只保留角色对话(dialogue)
              dialogue: generated.dialogue || fallbackText,
              narration: '', // 旁白已禁用,置空
          ...
          appendJsonl(stage5AJsonl, {
            ...
            batchIndex: batchIdx + 1,
            result: { batchIndex: batchIdx + 1, shotCount: normalized.length },
            memory: snapshotMemory('stage5A:batch-success', { batchIndex: batchIdx + 1 }),
          });
        } else {
          throw new Error(result.error || 'LLM返回无效数据');
        }
      } catch (err) {
        this.log('STAGE-5A', `⚠️ 批次 ${batchIdx + 1} 失败: ${err.message}`);

        appendJsonl(stage5AJsonl, {
          ts: new Date().toISOString(),
          batchIndex: batchIdx + 1,
          error: err.message,
          memory: snapshotMemory('stage5A:batch-failed', { batchIndex: batchIdx + 1 }),
        });

        const fallback = batch.map((scene) => {
          const fallbackText = this._buildFallbackDialogue(scene, input.characters);
          const fallbackBlock = this._buildFallbackDialogueBlock(scene, input.characters, fallbackText);
          return {
            ...scene,
            scene: scene.name || '',
            // v6.7.0-dialogue-patch: 支持 dialogueBlock 结构化对话
            dialogueBlock: fallbackBlock,
            // v6.5.65-P8-fix: 全局禁用旁白,只保留角色对话(dialogue)
            dialogue: fallbackText,
            narration: '', // 旁白已禁用,置空
            action: '', // v6.5.65-P8-patch-008: fallback空动作
            mouthAction: 'speaking_normal',
            emotionPhase: this._inferEmotionPhase(scene),
            scriptCoreSuccess: false,
            scriptCoreError: err.message
          };
        });
        ...
        try {
          ...
          this.log('STAGE-5A', `  ⚠️ checkpoint写入失败: batch ${batchIdx + 1} - ${e.message}`);
        }
  ...
  async _generateVisualPromptPhase(input) {
    const { LLMEngine } = require('../../systems/llm-reasoning-engine');
    const { memoryReliefPoint } = require('../../systems/runtime-memory-guard');
    // v6.6.3-fix: Stage 5B 新增 checkpoint 支持
    const { saveCheckpoint, loadCheckpoint, appendJsonl } = require('../../systems/pipeline-checkpoint');
    ...
    // v6.6.3-fix: 断点恢复 —— Stage 5B 之前完全没有 checkpoint，被杀后全部丢失
    let startIdx = 0;
    const saved = loadCheckpoint(stage5BCheckpoint, null);
    if (saved && Array.isArray(saved.results) && saved.completedShots > 0) {
      const completed = saved.completedShots;
      if (completed < scenes.length) {
        this.log('STAGE-5B', `🔄 发现断点: 已完成 ${completed}/${scenes.length} 镜头，从镜头 ${completed + 1} 恢复`);
        results.push(...saved.results);
        startIdx = completed;
      } else if (completed >= scenes.length) {
        this.log('STAGE-5B', `✅ 断点显示全部 ${completed} 镜头已完成，跳过 Stage 5B`);
        return saved.results;
      }
    }
    ...
    for (let i = startIdx; i < scenes.length; i++) {
      ...
      // v6.6.3-fix: 事件循环让出，防止进程被判定为僵死
      if (i > startIdx) await new Promise(r => setTimeout(r, 0));

      await memoryReliefPoint('stage5B:before-shot', {
        shotIndex: i + 1,
      ...
      // v6.5.65-P8-fix-2: maxTokens 1024→1536，给完整 JSON 响应留出空间
      // 保持单镜头循环 + 事件循环让出 + 强制 GC，避免 OOM
      const schema = {
        id: scene.id,
        visualPrompt: ''
      };
      ...
      if (!result) {
        this.log('STAGE-5B', `⚠️ ${scene.id} LLM返回null，使用fallback`);
        results.push({
          ...scene,
          visualPrompt: this._buildFallbackVisualPrompt(scene, world),
          visualPromptSuccess: false
        });
        appendJsonl(stage5BJsonl, { ts: new Date().toISOString(), shotIndex: i + 1, shotId: scene.id, success: false, reason: 'llm_null' });
        continue;
      }
      ...
      if (result.success && result.data?.id === scene.id) {
        ...
        appendJsonl(stage5BJsonl, { ts: new Date().toISOString(), shotIndex: i + 1, shotId: scene.id, success: true });
      } else {
        ...
        appendJsonl(stage5BJsonl, { ts: new Date().toISOString(), shotIndex: i + 1, shotId: scene.id, success: false, error: result.error });
      }

      // v6.6.3-fix: 每个镜头完成后立即保存 checkpoint
      try {
        saveCheckpoint(stage5BCheckpoint, {
          updatedAt: new Date().toISOString(),
          completedShots: i + 1,
          totalShots: scenes.length,
          results: results,
        ...
        this.log('STAGE-5B', `  ⚠️ checkpoint写入失败: ${scene.id} - ${e.message}`);
      }

      // v6.5.64-P2-fix: 强制释放内存,避免OOM
      if (global.gc) global.gc();
      // 让出事件循环,防止阻塞
      await new Promise(resolve => setTimeout(resolve, 100));

      await memoryReliefPoint('stage5B:after-shot', {
        shotIndex: i + 1,
        totalShots: scenes.length,
      });
    }
  ...
  _buildScriptCorePrompt(batch, core, world, input, batchIdx, totalBatches) {
    const { topic = '未指定', projectName = '', characters = [] } = input;
    const config = this._getTopicConfig(projectName, topic);
    const speaker = characters[0]?.name || '主讲人';

    // v6.6.8-fix: 组合方案 D+E+F
    // D: 主题锚定 + 边界护栏
    // E: 内容规划器 (内置于 prompt 中)
    // F: 正面引导替代负面约束
    ...
    if (config.episodeIndex > 1) {
      parts.push(`<previous_episodes>前 ${config.episodeIndex - 1} 集已覆盖：${config.forbiddenContext || '前期内容'}</previous_episodes>`);
    }
    parts.push(`</topic_context>`);

    // ---------------- E: 内容规划器 (每个场景明确内容方向) ----------------
    parts.push(`\n<content_plan>`);
    parts.push(`以下是为每个场景规划的内容方向。请严格按此方向生成台词，不得偏离：`);
    for (const scene of batch) {
      const requiredName = config.sceneNameMapping[scene.id] || scene.name;
      // 根据场景名称和类型自动生成内容规划
      const contentPlan = this._generateSceneContentPlan(scene, config, topic);
      parts.push(`<scene id="${scene.id}" name="${requiredName}">`);
      parts.push(`  <content_direction>${contentPlan}</content_direction>`);
      parts.push(`  <must_cover>${scene.requiredContent || config.topic || topic}</must_cover>`);
      parts.push(`</scene>`);
    }
    parts.push(`</content_plan>`);

    // ---------------- F: 正面引导 (替代负面约束) ----------------
    if (config.requiredKeywords && config.requiredKeywords.length > 0) {
      parts.push(`\n<required_topics>`);
      parts.push(`本集台词(dialogue)鼓励覆盖以下方向（每段台词至少对应 1-2 个）：`);
      for (const kw of config.requiredKeywords) {
        parts.push(`- ${kw}`);
      }
      parts.push(`</required_topics>`);
    }

    // ---------------- 输出示例 (正面示例) ----------------
    parts.push(`\n<example>正确示例：</example>`);
    parts.push(`场景 S02 名称固定为："运动过度-最常见原因"`);
    parts.push(`正确 dialogue："运动过度是横纹肌溶解最常见的原因。平时不锻炼的人突然进行大量剧烈运动，比如举重、长跑，肌纤维会大量损伤、破裂，细胞里的肌红蛋白就会漏到血液里。"`);
    parts.push(`说明：此 dialogue 围绕"原因和机制"展开，符合本集主题。`);

    parts.push(`\n<output_format>`);
    parts.push(`请返回严格 JSON，不要包含 markdown 代码块标记：\n`);
    parts.push(`{\n "scenes": [\n {\n "id": "S02",\n "scene": "运动过度-最常见原因",\n "dialogue": "运动过度是横纹肌溶解最常见的原因...",\n "action": "双手做出举重姿势，随后模拟跑步动作",\n "mouthAction": "speaking_normal",\n "emotionPhase": "serious"\n }\n ]\n}`);
    parts.push(`</output_format>`);

    // ---------------- 执行规则 (简化 + 正面引导) ----------------
    parts.push(`\n<rules>`);
    parts.push(`1. 必须直接使用 <content_plan> 中提供的 scene 名称，禁止修改。`);
    parts.push(`2. 每个 dialogue 必须严格遵循对应场景的 <content_direction>，不得偏离。`);
    parts.push(`3. 台词必须围绕本集主题"${config.topic || topic}"展开，可以讲解症状、检查、原因、机制等，按场景需要选择侧重点。`);
    ...
    // ---------------- 自我校验 (正面验证) ----------------
    parts.push(`\n<self_check>`);
    parts.push(`生成 JSON 后，请检查：\n- 是否所有 scene 名称与 <content_plan> 完全一致？\n- 是否每个 dialogue 都遵循了对应场景的 <content_direction>？\n- 是否台词围绕本集主题展开？\n- 如果检查发现偏离，请修正后重新生成。`);
    parts.push(`</self_check>`);

    return parts.join('\n');
  }

  // v6.6.8-fix: E-内容规划器 - 为每个场景生成正面内容方向
  _generateSceneContentPlan(scene, config, topic) {
    const sceneName = config.sceneNameMapping[scene.id] || scene.name || '';
    const sceneType = scene.type || 'explanation';
    
    // 根据场景类型和名称自动生成内容方向
    if (sceneType === 'intro' || sceneType === 'opening') {
      return `开场引入：简要介绍本集主题，建立观众兴趣，点明本集将要讲解的核心内容。`;
    }
    if (sceneType === 'ending' || sceneType === 'closing') {
      return `总结回顾：梳理本集核心要点，强调关键信息，给出实用建议或行动号召。`;
    }
    
    // 根据场景名称关键词匹配内容方向
    const nameLower = sceneName.toLowerCase();
    if (nameLower.includes('症状') || nameLower.includes('表现')) {
      return `症状讲解：详细描述该主题的典型症状、体征表现，帮助观众识别。`;
    }
    if (nameLower.includes('检查') || nameLower.includes('化验') || nameLower.includes('实验室')) {
      return `检查指标：介绍相关实验室检查项目、指标名称及临床意义。`;
    }
    if (nameLower.includes('原因') || nameLower.includes('机制') || nameLower.includes('为什么')) {
      return `原因分析：讲解发生机制、常见诱因、病理过程。`;
    }
    if (nameLower.includes('治疗') || nameLower.includes('处理')) {
      return `治疗方案：介绍治疗原则、常用方法、注意事项。`;
    }
    if (nameLower.includes('预防') || nameLower.includes('避免')) {
      return `预防措施：给出实用预防建议、生活方式指导。`;
    }
    if (nameLower.includes('运动') || nameLower.includes('锻炼')) {
      return `运动相关：讲解运动相关的知识点、注意事项、正确方法。`;
    }
    if (nameLower.includes('药物') || nameLower.includes('用药')) {
      return `药物知识：介绍相关药物、用药原则、注意事项。`;
    }
    
    // 默认：根据本集主题自动生成
    return `围绕主题"${topic}"展开讲解，重点突出本场景的核心知识点。`;
  }
  ...
  _getTopicConfig(projectName, topic) {
    ...
    // 如果没有项目名，尝试按主题关键词匹配
    for (const [key, cfg] of Object.entries(TOPIC_CONSISTENCY_CONFIG)) {
      if (topic && cfg.topic && topic.includes(cfg.topic)) {
        return cfg;
      }
    }
  ...
  _checkTopicDeviation(dialogue, config) {
    if (!dialogue || typeof dialogue !== 'string') return { score: 1, reasons: ['dialogue为空'] };
    const text = dialogue.toLowerCase();
    const reasons = [];

    // 检查是否包含明显的离题内容（如完全无关的话题）
    const offTopicPatterns = ['篮球', '足球', '股票', '政治', '军事'];
    let offTopicHits = 0;
    for (const pattern of offTopicPatterns) {
      if (text.includes(pattern)) offTopicHits++;
    }
    if (offTopicHits > 0) {
      return { score: 0.8, reasons: ['包含明显离题内容'], offTopicHits };
    }

    // 检查内容长度是否足够（空或过短视为偏离）
    if (text.length < 20) {
      return { score: 0.6, reasons: ['内容过短'] };
    }

    // 检查是否包含科普相关关键词（放宽标准）
    const medicalKeywords = ['横纹肌', '溶解', '症状', '检查', '医院', '治疗', '预防', '肌肉', '尿液', 'CK', '肌红蛋白', '肾功能', '电解质'];
    let medicalHits = 0;
    for (const kw of medicalKeywords) {
      if (text.includes(kw.toLowerCase())) medicalHits++;
    }
    
    // 只要命中 1 个以上医学关键词，就视为基本符合主题
    if (medicalHits >= 1) {
      return { score: 0, reasons: [], medicalHits };
    }

    // 如果没有命中任何医学关键词，给予中等偏离度
    return { score: 0.4, reasons: ['未命中医学关键词'], medicalHits };
  }

  // 校验整个剧本的主题一致性
  _validateTopicAdherence(scenes, input) {
    const config = this._getTopicConfig(input.projectName, input.topic);
    const results = [];
    ...
    for (const scene of scenes) {
      const check = this._checkTopicDeviation(scene.dialogueBlock?.text || scene.dialogue, config);
      results.push({ id: scene.id, name: scene.scene, ...check });
      totalDeviation += check.score;
      if (check.score > 0.4) failedScenes += 1; // 偏离度 > 0.4 视为失败
    }

    const avgDeviation = scenes.length ? totalDeviation / scenes.length : 0;
    // v6.6.8-fix: 大幅放宽验证阈值，避免过度修正
    const passed = failedScenes === 0 && avgDeviation < 0.5;
    
    // 如果验证未通过但偏离度不高，记录日志但不阻塞
    if (!passed && avgDeviation < 0.3) {
      this.log('STAGE-5', `⚠️ 主题偏离度 ${avgDeviation.toFixed(2)} 在可接受范围，跳过修正`);
      return { passed: true, avgDeviation, failedScenes, details: results, config };
    }

    return { passed, avgDeviation, failedScenes, details: results, config };
  }

  // 使用 LLM 对偏离场景进行主题修正（保留动作/表情，只改 dialogue）
  async _autoCorrectScene(scene, config, llm) {
    // v6.6.8-fix: 简化修正 prompt，避免 LLM 陷入长 reasoning
    const prompt = `<system>你是一位内容优化Agent。你只优化台词(dialogue)的表达方式，不修改场景名称和动作。</system>

<task>
原场景名称：${scene.scene}
原台词："""${scene.dialogueBlock?.text || scene.dialogue}"""

优化要求：
1. 确保台词紧密围绕"${config.topic}"展开。
2. 保持科普视频的口语化、讲解风格，由主讲人直接口述。
3. 专业术语后必须紧跟一句口语化解释。
4. 返回严格 JSON：{"dialogue": "优化后的台词"}
</task>`;
    ...
    try {
      ...
      if (result?.data?.dialogue) {
        return { ...scene, dialogue: result.data.dialogue };
      }
    } catch (e) {
      this.log('STAGE-5', `⚠️ 自动修正失败: ${e.message}`);
    }
    return scene;
  }

  // 批量修正：只修正偏离度高的场景 (v6.6.8-fix: 激进兜底 - 修正失败时返回原内容，最多修正1轮)
  async _autoCorrectScenes(scenes, validation, llm) {
    if (validation.passed) return scenes;

    const corrected = [];
    for (const scene of scenes) {
      const detail = validation.details.find(d => d.id === scene.id);
      if (detail && detail.score > 0.5) {
        this.log('STAGE-5', `🔧 尝试修正场景 ${scene.id} (${scene.scene})，偏离度: ${detail.score.toFixed(2)}`);
        try {
          const fixed = await this._autoCorrectScene(scene, validation.config, llm);
          if (fixed && fixed.dialogue && fixed.dialogue !== scene.dialogue) {
            this.log('STAGE-5', `✅ 场景 ${scene.id} 修正成功`);
            corrected.push(fixed);
            continue;
          }
        } catch (e) {
          this.log('STAGE-5', `⚠️ 场景 ${scene.id} 修正失败: ${e.message}，使用原内容`);
        }
        // v6.6.8-fix: 修正失败或无效，返回原内容
  ...
  // 主题偏离日志记录
  async _logTopicDeviation(scenes, validation, input) {
    const debugDir = '/root/.openclaw/workspace/debug_llm';
    try {
      const fs = require('fs');
      if (!fs.existsSync(debugDir)) fs.mkdirSync(debugDir, { recursive: true });
      const timestamp = Date.now();
      const file = `${debugDir}/${timestamp}_topic_deviation.json`;
      fs.writeFileSync(file, JSON.stringify({
        projectName: input.projectName,
        topic: input.topic,
        timestamp: new Date().toISOString(),
        avgDeviation: validation.avgDeviation,
        passed: validation.passed,
        details: validation.details,
        dialogues: scenes.map(s => ({ id: s.id, scene: s.scene, dialogue: s.dialogue }))
      }, null, 2));
    } catch (e) {
      this.log('STAGE-5', `⚠️ 记录偏离日志失败: ${e.message}`);
    }
  }

  // 从 LLM 结果中安全提取内容 (v6.6.8-fix: 删除禁用词清洗，避免误伤)
  _extractAndSanitizeContent(rawResult, config) {
    let data = rawResult?.data || rawResult?.content || rawResult;

    // 如果 content 为空但有 reasoning，尝试从 reasoning 的 JSON 块提取
    if (!data && rawResult?.reasoning) {
      const jsonMatch = rawResult.reasoning.match(/\{[\s\S]*\}/);
      if (jsonMatch) {
        try {
  ...
  _buildVisualPrompt(scene, core, world, idx, total, input) {
    const parts = [];
    // v6.6.4-root-fix: 教育片即使使用nirath模式，也不视为Nirath内容
    const isNirath = this.isNirathContent;
    const title = input?.title || input?.content?.title || '';
    const topic = input?.topic || input?.content?.topic || '';
    ...
    parts.push(`
【视频主题】`);
    parts.push(`标题:${title || '未指定'}`);
    parts.push(`主题:${topic || '未指定'}`);
...
【当前镜头】${idx + 1}/${total}`);
    ...
    // v6.5.65-P8-fix: 全局禁用旁白,不传递 narration 字段
...
【硬性约束】`);
    ...
    parts.push(`- 顶层只包含 id, visualPrompt, action`);
    parts.push(`- id 必须与输入一致`);
    parts.push(`- action 字段描述角色的具体动作(不含场景和台词)`);
...
【输出示例】`);
  ...
  _buildFallbackDialogue(scene, characters = {}) {
    const name = scene.name || scene.scene || '当前场景';
  ...
  _buildFallbackDialogueBlock(scene, characters = {}, text = '') {
    ...
    // 推断对话类型
    let type = '独白';
    if (scene.type === 'climax' || scene.emotionPhase === 'climax') type = '喊战';
    else if (scene.type === 'dialogue') type = '对白';
    else if (scene.type === 'opening' || scene.type === 'intro') type = '独白';
    ...
    const emotion = emotionMap[scene.emotionPhase] || '平静';

    // 推断方式
    let manner = '平静地说';
    if (type === '喊战') manner = '大声喊道';
    else if (emotion === '激动' || emotion === '愤怒') manner = '激动地说道';
    else if (emotion === '平静') manner = '平静地说';
    else if (emotion === '紧张') manner = '紧张地说道';
    ...
    return {
      ...
      emotion,
      target: '',
      timing: scene.action ? `在${scene.action.substring(0, 40)}...时` : '',
      manner,
      type,
    };
  }
...
${isNirath
  ? `{"scenes":[{"id":"S01","scene":"场景名称","dialogueBlock":{"speaker":"说话角色名","text":"角色对白或台词文本(不要旁白叙述,要角色自己说的话)","emotion":"情绪状态","target":"对话对象","timing":"与动作配合时机","manner":"说话方式","type":"独白/对白/旁白/喊战/内心OS"},"action":"角色具体动作描述,如双手展开、边走边转身","type":"opening","characters":["xiaoG","taotie"],"mouthAction":"speaking_whisper","emotionPhase":"curiosity","importance":8,"visualComplexity":7,"visualPrompt":"超写实,电影级光影,角色动作描述(300-500字)","beastDialogue":"异兽台词(如有,20字内)"}]}`
  : `{"scenes":[{"id":"S01","scene":"场景名称","dialogueBlock":{"speaker":"说话角色名","text":"角色对白或台词文本(不要旁白叙述,要角色自己说的话)","emotion":"情绪状态","target":"对话对象","timing":"与动作配合时机","manner":"说话方式","type":"独白/对白/旁白/喊战/内心OS"},"action":"角色具体动作描述,如双手展开、边走边转身","type":"explanation","characters":["${charList.split(',')[0] || 'chen-nurse'}"],"mouthAction":"speaking_normal","emotionPhase":"professional","importance":8,"visualComplexity":7,"visualPrompt":"超写实,电影级光影,角色动作描述(300-500字)"}]}`
}

## 关键规则(P0级约束)
- ❌ 绝对禁止生成旁白/叙述性文字(如"小G来到了...")
- ✅ 必须生成角色对白/台词(角色自己说的话)
- ✅ 必须明确标注说话者(speaker字段),不可省略
- ✅ dialogueBlock 7个字段全部必填: speaker, text, emotion, target, timing, manner, type
...
- 对话类型选择指南:
  ...
  async stageStoryboard(script, durations, input = {}, characters = {}) {
    ...
    for (let i = 0; i < mappedScenes.length; i++) {
      ...
      // v6.7.0-patch: 完整映射 LLM 25字段输出到 shot 对象（双命名兼容）
      const llmShot = llmStoryboard?.[i] || {};
      const shot = {
        id: scene.id || `S${String(i + 1).padStart(2, '0')}`,
        scene: llmShot.scene || scene.scene || 'default',
        // v6.7.0-dialogue-patch: 支持 dialogueBlock 结构化对话
        dialogueBlock: llmShot.dialogueBlock || scene.dialogueBlock || null,
        dialogue: scene.dialogue || llmShot.dialogue || (llmShot.dialogueBlock?.text || '') || (scene.dialogueBlock?.text || ''),
        narration: scene.narration || scene.dialogue || '',
        duration,
        type: scene.type || 'explanation',
        characters: scene.characters || [],
        // v6.5.64-P0: 优先使用LLM生成的动作和表情
        mouthAction: llmShot.mouthAction || scene.mouthAction || this.generateDefaultMouthAction(scene.type, i === 0),
        // v6.5.65-P8-patch-008-fix: 注入 action 字段(从剧本透传或LLM生成)
        action: llmShot.action || scene.action || llmShot.characterAction || '',
        importance: scene.importance || 5,
        visualComplexity: scene.visualComplexity || 5,
        emotionPhase: llmShot.emotionPhase || llmShot.mood || scene.emotionPhase || this.calculateEmotionPhase(i, mappedScenes.length),
        // v6.5.64-P0: 优先使用LLM生成的视觉描述
        visualPrompt: llmShot.visualPrompt || scene.visualPrompt || '',
        fpvRecommended: scene.fpvRecommended || false,
        fpvScore: scene.fpvScore || 0,
        fpvReason: scene.fpvReason || '',
        cameraMovement: llmShot.cameraMovement || llmShot.camera_movement || null,
        prompt: null,
        // v6.5.64-P0: 标记LLM生成
        llmEnhanced: !!llmStoryboard?.[i],

        // ========== v6.7.0-patch: 25字段完整映射（toStandardPrompt 读取的属性名）==========
        directorStyle: llmShot.director_instruction || llmShot.directorInstruction || '',
        constraint: llmShot.constraint || '',
        baseline: llmShot.baseline || '',
        negativePrompt: llmShot.negative || '',
        characterProfile: llmShot.consistency || '',
        composition: llmShot.composition || '',
        colorPalette: llmShot.color_palette || llmShot.colorPalette || '',
        depthOfField: llmShot.depth_of_field || llmShot.depthOfField || '',
        mood: llmShot.mood || '',
        brightConstraint: llmShot.bright_constraint || llmShot.brightConstraint || '',
        characterConstraint: llmShot.character_constraint || llmShot.characterConstraint || '',
        costume: llmShot.costume || '',
        props: llmShot.props || '',
        pacing: llmShot.pacing || '',
        audio: llmShot.audio || '',
        makeup: llmShot.makeup || '',
        transition: llmShot.transition || '',
        // lighting 保留对象结构（toStandardPrompt 读取 .string/.keyLight），同时挂纯文本
        lighting: typeof llmShot.lighting === 'string'
          ? { string: llmShot.lighting, keyLight: { effect: llmShot.lighting } }
          : (llmShot.lighting || null),

        // ========== v6.7.0-patch: 25字段完整映射（FieldCheckAgent 读取的 snake_case 属性名）==========
        // FieldCheckAgent 通过 shot[spec.name_en] 读取，必须用 snake_case
        director_instruction: llmShot.director_instruction || llmShot.directorInstruction || '',
        camera_movement: typeof llmShot.camera_movement === 'string'
          ? llmShot.camera_movement
          : (llmShot.cameraMovement?.string || llmShot.cameraMovement?.description || llmShot.camera_movement || ''),
        character: (scene.characters || []).join(',') || llmShot.character || '',
        negative: llmShot.negative || '',
        portraits: llmShot.portraits || '',
        consistency: llmShot.consistency || '',
        color_palette: llmShot.color_palette || llmShot.colorPalette || '',
        depth_of_field: llmShot.depth_of_field || llmShot.depthOfField || '',
        timeline: llmShot.timeline || '',
        bright_constraint: llmShot.bright_constraint || llmShot.brightConstraint || '',
        character_constraint: llmShot.character_constraint || llmShot.characterConstraint || '',
      };
  ...
  async stageNarrationTrim(storyboard, durations) {
    // v6.5.65-P8-fix: 全局禁用narration,强制清除所有镜头的 narration 字段
    const shots = storyboard.shots || [];
    let clearedCount = 0;
    ...
    this.log('STAGE-7.3', `⏭️ 全局禁用narration,强制清除 ${clearedCount}/${shots.length} 个镜头的 narration 字段`);
    return { trimmedCount: 0, totalTrimmedChars: 0, skipped: true, reason: '全局禁用narration', clearedCount };
  }
  ...
  async stageOpeningGeneration(input, storyboard, characters) {
    // v6.6.4-root-fix: 尊重 hasOpening 配置，false 时不生成片头
    if (input.hasOpening === false || input.opening?.enabled === false || input.noOpening === true) {
      this.log('STAGE-7.5', '⏭️ hasOpening=false，跳过片头生成');
      return null;
    }
    ...
    if (this.mode === 'nirath') {
      ...
      // v6.7.0-fix: opening-system-v3.js 导出的是函数对象,不是类
      const protagonistId = Object.keys(input.characters || {})[0] || 'xiaoG';
      const characterIds = Object.keys(input.characters || {});
      const featuredBeastId = characterIds.length > 1 ? characterIds[1] : (input.beast?.id || 'tao-tie');
      openingResult = OpeningSystem.generateOpeningV3({
        episodeTitle: input.projectName || input.title || '山海经:异兽志',
        episodeTheme: input.theme || 'epic',
        episodeSummary: input.episodeSummary || input.content?.topic || '',
        protagonistId: protagonistId,
        featuredBeastId: featuredBeastId,
      ...
      // v6.5.65-P8-fix: 清除模块缓存,确保加载最新版本
      const gosPath = require.resolve('../../systems/generic-opening-system.js');
      delete require.cache[gosPath];
      const GenericOpeningSystem = require('../../systems/generic-opening-system.js');
      const openingSystem = new GenericOpeningSystem({
        mode: 'generic',
        duration: input.openingDuration || 9 // v6.5.65-P8-fix: 默认9秒匹配验证器要求
      });
      openingResult = openingSystem.generateOpening(input, storyboard, characters);
    }

    // v6.5.65-P8-fix: 确保通用片头字段完整(兼容 Stage 11 和完整性验证器)
    if (openingResult) {
      if (!openingResult.scene) openingResult.scene = '片头-开场';
      if (!openingResult.mouthAction) openingResult.mouthAction = '';
      if (!openingResult.title || typeof openingResult.title !== 'object') {
        openingResult.title = {
          main: input.projectName || '未命名项目',
          sub: '',
      ...
      // v6.7.0-fix: opening-system-v3.js 返回的 characters 是对象 {protagonist, beast}
      // 需要转换为字符串数组以兼容下游 Stage-8
      const charObj = openingResult.characters;
      if (charObj && typeof charObj === 'object' && !Array.isArray(charObj)) {
        openingResult.characters = [
          charObj.protagonist?.id || charObj.protagonist?.name,
          charObj.beast?.id || charObj.beast?.name
        ].filter(Boolean);
      }
    }
  ...
  extractOpeningConfig(input, storyboard, characters) {
    const config = {
      episodeTitle: input.projectName || input.title || input.content?.title || '山海经:异兽志',
  ...
  async stageStoryboardValidation(storyboard, input) {
    ...
    try {
      ...
      // v6.7.0-fix7: 校验器调用失败时不伪造通过，改为待审查状态
      validation = {
        valid: false, // 不再默认true
        errors: [{ type: 'validator_error', message: `校验器异常: ${e.message}`, severity: 'error' }],
        warnings: [{ message: `StoryboardValidator调用失败: ${e.message}` }]
      };
    }
    ...
    // v6.2-patch71-fix: 时长硬约束检查--动态上限,使用校准后的时长
    const allocatedDurations2 = storyboard.shots?.map(s => s.duration).filter(Boolean) || [];
    const maxAllocatedDuration2 = allocatedDurations2.length > 0 ? Math.max(...allocatedDurations2) : 15;
    const durationUpperLimit2 = Math.max(maxAllocatedDuration2, 15); // 至少15秒上限,允许教育类长镜头
    ...
    // v6.7.0-fix7: 严格校验——如果pipeline发现了error，强制valid=false
    // 不再盲目信任storyboardValidator的valid状态
    const pipelineErrors = (validation.errors || []).filter(e => e.severity === 'error');
    const hasPipelineErrors = pipelineErrors.length > 0;
    // 只有当validator明确返回valid=true且pipeline无error时，才认为有效
    const validatorExplicitValid = validation.valid === true;
    validation.valid = validatorExplicitValid && !hasPipelineErrors;
    // 如果validator没有明确返回valid=true，但至少没有pipeline error，标记为待审查
    if (!validatorExplicitValid && !hasPipelineErrors) {
      validation.warnings = validation.warnings || [];
      validation.warnings.push({
        type: 'validator_uncertain',
        message: 'StoryboardValidator未明确返回有效状态，建议人工审查',
        severity: 'warning'
      });
    }
  ...
  async stageCameraMovement(storyboard, fpvDecision, durations) {
    ...
    // v6.7.0-fix9: 深拷贝storyboard，防止Stage 9修改污染Stage 11的原始数据
    const storyboardCopy = JSON.parse(JSON.stringify(storyboard));
    const shots = storyboardCopy.shots || [];
    ...
    for (const shot of shots) {
      ...
      if (shot.fpvRecommended || shot.shotType === 'fpv' || shot.type === 'fpv') {
        ...
        // v6.2-fix: 非FPV镜头使用v3完整运镜系统 — v6.6.6-fix-fix: 教育片也启用v3时间轴(支持EDU/educational大小写)
        if (this.mode === 'nirath' || this.input?.videoType === 'educational' || this.input?.videoType === 'EDU' || this.input?.filmType === 'educational' || this.input?.filmType === 'EDU') {
          movement = this.generateV3CameraMovement(shot, timelineGenerator, sceneTypeToTransition, emotionToLighting, emotionToSpeedCurve);
        } else {
  ...
  generateV3CameraMovement(shot, timelineGenerator, sceneTypeToTransition, emotionToLighting, emotionToSpeedCurve) {
    ...
    // 根据场景类型选择景别切换策略 - v6.6.6-fix: 引入镜头序号和场景内容增强差异化
    // 避免所有镜头使用相同的transitionType/lightingType/speedCurve
    const shotIndex = parseInt((shot.id || '').replace(/\D/g, ''), 10) || 0;
    const transitionTypePool = ['progressive_reveal', 'orbit_explore', 'impact_shock', 'dialogue_dance', 'poetic_wander'];
    const transitionType = sceneTypeToTransition[shot.type] || transitionTypePool[shotIndex % transitionTypePool.length];
    
    // 根据情绪阶段选择灯光变化 - v6.6.6-fix: 基于场景内容差异化
    const sceneKeywords = (shot.scene || '').toLowerCase();
    let lightingType = emotionToLighting[phase] || 'dawn_break';
    if (sceneKeywords.includes('开场') || sceneKeywords.includes('intro') || sceneKeywords.includes('片头')) {
      lightingType = 'dawn_break';
    } else if (sceneKeywords.includes('症状') || sceneKeywords.includes('检查') || sceneKeywords.includes('实验室')) {
      lightingType = 'spotlight_drama';
    } else if (sceneKeywords.includes('结尾') || sceneKeywords.includes('close') || sceneKeywords.includes('总结')) {
      lightingType = 'emotion_temperature';
    } else if (sceneKeywords.includes('核心') || sceneKeywords.includes('讲解')) {
      lightingType = 'spotlight_drama';
    }
    ...
    if (!timeline || !timeline.segments) {
      ...
      return this.modules.cameraMovement.generateNirathMovement(sceneName, phase, {
  ...
  // v3 优先：v2 仅作为 description 回退，避免重复计算
  // 先尝试 v3 时间轴生成，v2 的 description 只在 v3 失败时回退使用
  let baseMovement = null;
  try {
    baseMovement = this.modules.cameraMovement.generateNirathMovement(sceneName, phase, {
      shotSize: shot.shotSize,
      movement: shot.movement,
      speed: shot.speed
    });
  } catch (e) {
    baseMovement = { description: '', shotSize: 'medium', movement: 'static', speed: 'normal', cameraAngle: 'eye-level', lensFocalLength: '50mm' };
  }

    // v6.6.6-fix: 轻量级timeline存储，避免内存膨胀
    const slimTimeline = {
      totalDuration: timeline.totalDuration,
      segmentCount: timeline.segmentCount,
      transitionName: timeline.transitionName,
      lightingName: timeline.lightingName,
      speedCurveName: timeline.speedCurveName,
    ...
    const movement = {
      ...
      shotSize: baseMovement.shotSize,
      movement: baseMovement.movement,
      speed: baseMovement.speed,
      cameraAngle: baseMovement.cameraAngle,
      lensFocalLength: baseMovement.lensFocalLength,
    ...
    // v6.6.6-fix: 轻量级存储到shot，避免内存膨胀
    shot._segmentCount = timeline.segmentCount;
    shot._timelineSummary = {
      segmentCount: timeline.segmentCount,
      transitionName: timeline.transitionName,
      lightingName: timeline.lightingName,
      speedCurveName: timeline.speedCurveName
  ...
  async stageContinuity(storyboard) {
    ...
    let continuity;
    try {
      // v6.7.0-fix6: 优先调用analyze方法（主入口），回退check/validate
      if (typeof this.modules.continuityEngine.analyze === 'function') {
        continuity = this.modules.continuityEngine.analyze(storyboard.shots);
      } else if (typeof this.modules.continuityEngine.check === 'function') {
    ...
    this.log('STAGE-10', `✅ 连续性检查 | 问题: ${continuity.issues?.length || 0} | 警告: ${continuity.warnings?.length || 0}`);
    return continuity;
  }
  ...
  _extractSegmentsFromShot(enhanced, shot) {
    ...
    if (shot && Array.isArray(shot._segments) && shot._segments.length > 0) {
      // v6.6.6-fix: _segments 已废弃，从 cameraMovement.timeline 获取
  ...
  async stageSafetyGate(stages) {
    ...
    for (let i = 0; i < stages.storyboard.shots.length; i++) {
      ...
      // 检查1: 台词/旁白是否非空(核心输入) - v6.6.2: 改为同时检查dialogue和narration
      const 核心文本 = shot.dialogue || shot.narration || '';
      if (!核心文本 || 核心文本.trim().length === 0) {
        errors.push(`台词/旁白为空`);
      }
      ...
      // 检查5: 时长是否合理(动态上限,尊重校准后的时长)
      const allocatedDurations = stages.duration?.durations || [];
      const maxAllocatedDuration = allocatedDurations.length > 0
        ? Math.max(...allocatedDurations.map(d => d.duration || 0))
        : 15;
      const durationUpperLimit3 = Math.max(maxAllocatedDuration, 15); // 至少15秒上限,允许教育类长镜头
      ...
      results.push({
        ...
        errors,
        inputStatus: {
          hasNarration: !!((shot.dialogue || shot.narration || '').trim()),
  ...
  const { memoryReliefPoint, snapshotMemory } = require('../../systems/runtime-memory-guard');
  const { updateShotPromptIncrementally, appendJsonl, loadCheckpoint } = require('../../systems/pipeline-checkpoint');
    this.log('STAGE-11', `渲染核心${this.mode === 'nirath' ? '(Nirath v24)' : '(v6.5.64-P0: LLM驱动)'}`);

    const prompts = [];
    const { storyboard, characters, camera } = stages;

    // v6.5.64-P2-fix: 预生产模式降低maxTokens避免OOM,不跳过Stage 11
    const isPreProduction = this.projectConfig?.isPreProduction || false;
    const maxTokens = isPreProduction ? 2048 : 4096;
    ...
    if (this.mode !== 'nirath') {
      try {
        const { result, driver, attempts } = await this.llmEnforcer.requireLLM(
          ...
          {
            llmEngine: this._createLLMEngine({ maxTokens }),
            llmOptions: { maxTokens, temperature: 1 } // v6.6.6-fix-fix: kimi-k2p6 requires temperature=1
          }
        );
    ...
    // v6.6.3-fix: 断点恢复 —— 加载已完成的shot，跳过重复工作
    const saved11 = loadCheckpoint(stage11Checkpoint, null);
    ...
    for (let i = 0; i < storyboard.shots.length; i++) {
      ...
      if (completedShotMap.has(fingerprint)) {
        ...
        if (savedShot && savedShot.prompt) {
          prompts.push({
            ...
            length: savedShot.prompt.length,
            utilization: Math.round((savedShot.prompt.length / 1500) * 100),
            utilizationStatus: savedShot.prompt.length >= 970 && savedShot.prompt.length <= 1500 ? '🔥理想' : (savedShot.prompt.length > 1500 ? '❌超标' : '⚠️空间浪费'),
            enhanced: true,
            cameraMovement: shot.cameraMovement || { movement: 'stable', description: '默认镜头' },
      ...
      // v6.6.1-fix: 事件循环让出，防止进程被判定为僵死
      if (i > 0) await new Promise(r => setTimeout(r, 0));
      
      await memoryReliefPoint('stage11:before-shot', {
        shotIndex: i + 1,
      ...
      if (shot.id === 'S00' && shot.isOpening && shot.prompt) {
        ...
        prompts.push({
          ...
          enhanced: true,
          isOpening: true,
          // v6.5.65-P8-fix: 片头不引用未初始化的 cameraObj
          cameraMovement: shot.cameraMovement || { movement: 'stable', description: '片头固定镜头' },
          // v6.5.65-P8-fix: 确保片头有 scene 字段(兼容完整性验证器)
          scene: shot.scene || '片头-开场',
          // v6.5.65-P8-fix: 保留片头 title 对象
          title: shot.title || { main: '', sub: '' },
          // v6.37-fix: 添加可选字段
        ...
        try {
          appendJsonl(stage11Jsonl, { shotId: shot.id, type: 'opening', prompt: openingPrompt, timestamp: Date.now() });
        } catch (e) { /* 静默忽略checkpoint错误 */ }
      ...
      if (this.isNirathContent) {
        ...
        // v6.5.65-P8-fix: 全局禁用旁白,只使用角色对话(dialogue)作为台词
        ...
        const renderResult = this.modules.renderCore.buildPromptV3({
          ...
          narration: narration,
          // v6.7.0-dialogue-patch: 传递结构化对话数据
          dialogueBlock: shot.dialogueBlock || null,
          ...
          action: shot.action || shot.actionDescription || '', // ✅ v6.5.65-P8-fix: 注入动作描述
          mouthAction: shot.mouthAction,
      ...
      if (isVisualEmpty) {
        ...
        // v6.6.6-fix: 统一时间轴--从 cameraMovement.timeline 生成prompt中的【镜头时间轴】
        const timelineSegments = shot.cameraMovement?.timeline?.segments;
        if (timelineSegments && timelineSegments.length > 0 && !prompt.includes('【镜头时间轴】')) {
          const timelineText = timelineSegments.map((seg, idx) =>
            `${idx + 1}. ${seg.timeRange || seg.time || ''}: ${seg.movement || seg.camera || seg.action || '固定镜头'}${seg.note ? `(${seg.note})` : ''}`
          ).join(';');
          prompt += ` 【镜头时间轴】${timelineText}`;
          this.log('STAGE-11', `  ⏱️ 时间轴同步: ${shot.id} | 从timeline生成 | ${timelineSegments.length}段`);
        }

        // v6.7.0-patch: 负面约束/角色约束/灯光兜底为P0必填，所有片型都必须注入（原逻辑教育片被误关）
        // --- 负面约束 ---
        if (!prompt.includes('【负面约束】') && !prompt.includes('【全局负面约束】')) {
          const negativeConstraint = '【负面约束】no text, no watermark, no blurry, no extra limbs, no deformed hands, no duplicate characters, no cartoon, no anime, no low quality, no dark shadows on face, no unnatural eye colors, no red eye, no blue pupil, no glowing eyes; eyes must be natural human black pupils; no crystal; no duplicate characters';
        ...
        // --- 灯光兜底（教育片也需）---
        if (!prompt.includes('【灯光')) {
          const fallbackLight = '【灯光】key light from front-right 45°, fill light left side, 5500K daylight, soft diffusion, 2:1 light ratio, even illumination on face';
          prompt += ` ${fallbackLight}`;
          this.log('STAGE-11', `  💡 灯光兜底注入: ${shot.id}`);
        }
        ...
        // v6.6.2: 结尾情绪增强（可配置）
        if (this.enhancementConfig.closingBoost && this.modules.closingBooster) {
        ...
        // v6.6.2: 叙事弧线注入（可配置）
        if (this.enhancementConfig.narrativeArc) {
        ...
        prompt = promptResult;
        // v6.6.6-fix: 删除直接追加 movement.description 的代码
        // toStandardPrompt 会负责从 shot.cameraMovement.description 生成 【运镜】字段
        // 如果在这里直接追加，会导致运镜内容重复出现（裸文本 + 标记文本各一次）
      ...
      // v6.6.2: 美术布景增强（可配置）
      if (this.enhancementConfig.setDesign && this.modules.setDesignModule) {
        try {
      ...
      // v6.6.2: 动作增强（可配置）
      if (this.enhancementConfig.motion && (this.modules.microMotionAdapter || this.modules.beastMotionAdapter)) {
        try {
      ...
      for (const sel of selectedAngles) {
        ...
        const angleDescMap = {
          'front': '正面', 'three-quarter': '侧面', 'closeup': '近景', 'side': '另一侧面'
        };
      ...
      if (imageRefLines.length > 0 && !prompt.includes('@image')) {
        ...
        // v6.6.6-fix: 将 @image 引用包装为 【绑定定妆照】标记，确保稳定裁剪不会删除
        const imageRefBlock = `【绑定定妆照】${imageRefText}`;
        if (prompt.length + imageRefBlock.length + 2 <= 1500) {
          prompt += ` ${imageRefBlock}`;
        } else {
          // 如果空间不足,裁剪尾部非核心内容来容纳 @image 引用
          const remaining = 1500 - imageRefBlock.length - 2;
          if (remaining > 100) {
            prompt = this.smartTrim(prompt, remaining, {
              preserve: ['视觉', '叙事', '台词', '嘴部动作', '镜头时间轴', '绑定定妆照'],
              trim: ['辅助运镜', '光影细节补充', '环境质感', '环境音效', '技术规格', '照明方案']
            });
            prompt += ` ${imageRefBlock}`;
          }
      ...
      // v6.7.0-patch: 第二段循环完成后统一设置shot.referenceImages，避免双重注入
      shot.referenceImages = referenceImages;
      shot.content = shot.content || [];
      for (const refImg of referenceImages) {
        if (!shot.content.some(c => c.type === 'image_url' && c.image_url?.url === refImg.image_url?.url)) {
          shot.content.push(refImg);
        }
      }

      // v6.6.6-fix: 注入 appearanceAnchor 覆盖 LLM 生成的角色服装描述
      const anchorCharId = (shot.characters || [])[0];
      if (anchorCharId) {
        const charCard = this._loadCharacterCard(anchorCharId);
        if (charCard?.appearanceAnchor?.uniform) {
          const anchorText = charCard.appearanceAnchor.uniform;
          // 如果 prompt 的 【角色】 字段包含 "职业套装" 或 "西装"，强制替换为 appearanceAnchor
          if (prompt.includes('职业套装') || prompt.includes('西装') || prompt.includes('套装')) {
            prompt = prompt.replace(/([【角色】][^【|]*)?(职业套装|西装|套装)([^【|]*)/, `【角色】${anchorText}`);
            this.log('STAGE-11', `  👔 服装锁定注入: ${shot.id} | ${anchorText.substring(0, 30)}...`);
          } else if (!prompt.includes(anchorText.substring(0, 10))) {
            // 如果完全没有包含 appearanceAnchor 的前 10 个字符，强制追加
            const anchorBlock = `【角色】${anchorText}`;
            if (prompt.length + anchorBlock.length + 2 <= 1500) {
              prompt += ` ${anchorBlock}`;
            } else {
              const remaining = 1500 - anchorBlock.length - 2;
              if (remaining > 100) {
                prompt = this.smartTrim(prompt, remaining, {
                  preserve: ['视觉', '叙事', '台词', '嘴部动作', '镜头时间轴', '绑定定妆照', '角色'],
                  trim: ['辅助运镜', '光影细节补充', '环境质感', '环境音效', '技术规格', '照明方案']
                });
                prompt += ` ${anchorBlock}`;
              }
      ...
      // v6.5.65-P8-fix-2: 计算全局时间轴偏移
      let currentGlobalTime = 0;
      for (let j = 0; j < i; j++) {
        currentGlobalTime += storyboard.shots[j].duration || 0;
      }
      shot._globalStartTime = currentGlobalTime;
      shot._globalEndTime = currentGlobalTime + (shot.duration || 0);
      ...
      // v6.6.2: Prompt 稳定性保护（外部专家审计修复）
      // 在最终输出前，确保关键字段不丢失
      const stabilityCheck = 关键字段存在(prompt);
      const lostFields = [];
      if (!stabilityCheck.对话指令 && (shot.dialogue || shot.narration || (shot.dialogueBlock?.text))) lostFields.push('对话指令');
      if (!stabilityCheck.动作 && shot.action) lostFields.push('动作');
      if (!stabilityCheck.时间轴 && shot.duration) lostFields.push('时间轴');
      if (!stabilityCheck.角色 && shot.characters?.length > 0) lostFields.push('角色');
      if (!stabilityCheck.场景 && shot.scene) lostFields.push('场景');

      if (lostFields.length > 0) {
        this.log('STAGE-11', `  🔒 稳定性保护: ${shot.id} | 恢复丢失字段: ${lostFields.join(',')}`);
        prompt = 最小补洞(prompt, shot);
        prompt = 恢复关键块(prompt, prompt, 1500);
        shot.prompt = prompt;
      }
      ...
      // v6.5.65-P8-fix: 确保 S00 有 scene 字段(兼容完整性验证器)
      if (shot.id === 'S00' && !shot.scene) {
        shot.scene = '片头-开场';
      }
      ...
      // v6.37-fix: 格式化 dialogue 字段
      const dialogueFormatted = this._formatDialogue(shot.dialogue, shot.narration, shot.characters, shot);
      ...
      // v6.6-fix: checkpoint每个shot
      try {
        updateShotPromptIncrementally(stage11Checkpoint, shot.id, prompt, {
          duration: shot.duration,
          action: shot.action,
        ...
        appendJsonl(stage11Jsonl, {
          ts: new Date().toISOString(),
          shotIndex: i + 1,
          shotId: shot.id,
          promptLength: prompt.length,
          memory: snapshotMemory('stage11:shot-success', {
            shotIndex: i + 1,
            shotId: shot.id,
          }),
        });
      } catch (e) {
        this.log('STAGE-11', `  ⚠️ checkpoint写入失败: ${shot.id} - ${e.message}`);
      }
      
      // v6.6.1-fix: 事件循环让出，防止进程被判定为僵死
      await new Promise(r => setTimeout(r, 0));
    }
    await memoryReliefPoint('stage11:end');
  ...
  _autoSelectPortraits(shot, characters) {
    try {
      ...
      for (const rawCharId of (shot.characters || [])) {
        ...
        // v6.5.65-P8-fix: 检查 portraits 是否有效(非空且有实际路径),而非仅检查是否存在
        const hasValidPortraits = char.portraits &&
          Object.values(char.portraits).some(p => p && typeof p === 'string' && p.length > 0);

        // 如果角色已有有效 portraits 且不是多风格模式,跳过
        if (hasValidPortraits && !char.enableMultiStyle) continue;

        // 根据场景选择风格和角度
        const scene = shot.scene || 'default';
        ...
        if (selectedPortrait) {
          ...
          this.log('STAGE-11', `  📸 自动选择定妆照: ${rawCharId} | 场景:${scene} | 角度:${angle} | 路径:${selectedPortrait}`);
        }
      }
    } catch (err) {
      // 静默失败,不影响主链路
   ...
   * v6.6.6-fix: 加载角色档案
   */
  _loadCharacterCard(charId) {
    const fs = require('fs');
    const path = require('path');
    const cardPath = path.join('/root/.openclaw/workspace', 'characters', charId.replace('chen-zhuo', 'chenzhuo'), 'character-card.json');
    try {
      ...
      this.log('STAGE-11', `  ⚠️ 加载角色档案失败: ${charId} | ${e.message}`);
    }
    return null;
  }
  ...
  _formatDialogue(dialogue, narration, characters, shot) {
    // v6.7.0-dialogue-patch: 优先使用 dialogueBlock 结构化数据
    const block = this._buildDialogueBlock(shot || {});
    if (block.text) {
      const speaker = block.speaker || '角色';
      const type = block.type || '独白';
      const emotion = block.emotion || '平静';
      return `${speaker}|${type}|${emotion}|${block.text}|LIP_SYNC:YES`;
    }
  ...
  /**
   * v6.7.0-dialogue-patch: 从 shot 构建 DIALOGUE_BLOCK 结构体
   */
  _buildDialogueBlock(shot) {
    const safe = (v) => (typeof v === 'string' ? v.trim() : '');

    // 如果 shot 已经有 dialogueBlock 对象，直接使用
    if (shot.dialogueBlock && typeof shot.dialogueBlock === 'object' && shot.dialogueBlock.text) {
      return {
        speaker: shot.dialogueBlock.speaker || '',
        text: shot.dialogueBlock.text || '',
        emotion: shot.dialogueBlock.emotion || '',
        target: shot.dialogueBlock.target || '',
        timing: shot.dialogueBlock.timing || '',
        manner: shot.dialogueBlock.manner || '',
        type: shot.dialogueBlock.type || '独白',
      };
    }

    // 从扁平 dialogue 字段构建
    const text = safe(shot.dialogue || shot.narration);
    if (!text) {
      return { speaker: '', text: '', emotion: '', target: '', timing: '', manner: '', type: '' };
    }
    ...
    if (shot.characters && shot.characters.length > 0) {
      speaker = shot.characters[0];
      // 尝试从角色数据获取真实名称
      if (this.characters?.[speaker]?.profile?.name) {
        speaker = this.characters[speaker].profile.name;
      }
    }
    // 如果 dialogue 文本以 "XXX：" 或 "XXX:" 开头，提取说话者
    ...
    // 推断对话类型
    let type = '独白';
    if (shot.interactionType === 'dialogue') type = '对白';
    else if (shot.type === 'climax' || shot.emotionPhase === 'climax') type = '喊战';
    else if (shot.type === 'opening') type = '独白';
    ...
    let timing = '';
    if (shot.action) {
      const actionPreview = safe(shot.action).substring(0, 60);
      timing = `在${actionPreview}...时`;
    }

    // 推断方式（基于 type 和 emotion）
    let manner = '';
    if (type === '喊战') manner = '怒吼';
    else if (type === '对白') manner = '说道';
    else if (emotion === '激动' || emotion === '愤怒') manner = '大声喊道';
    else if (emotion === '平静') manner = '平静地说';
    else manner = '说道';
  ...
  /**
   * v6.7.0-dialogue-patch: 将 DIALOGUE_BLOCK 渲染为自然语言【对话指令】
   */
  _renderDialogueInstruction(block, shot) {
    const safe = (v) => (typeof v === 'string' ? v.trim() : '');

    if (!block || !block.text) return '';

    const speaker = safe(block.speaker);
    const text = safe(block.text);
    const emotion = safe(block.emotion);
    const target = safe(block.target);
    const timing = safe(block.timing);
    const manner = safe(block.manner);
    const type = safe(block.type);

    // 根据 Seedance 官方最佳实践构建自然语言指令
    const parts = [];
    ...
    if (speaker) {
      let prefix = speaker;
      if (timing) {
        prefix += timing.replace(/^在/, '，在').replace(/时$/, '时，');
      }
      parts.push(prefix);
    }

    // 2. 情绪修饰 + 说话方式
    let verbPhrase = manner || '说道';
    if (emotion && !manner) {
      const emotionVerbMap = {
        '挑衅': '挑衅地喊道',
        '愤怒': '愤怒地吼道',
        '激动': '激动地喊道',
        '紧张': '紧张地说道',
      ...
      verbPhrase = emotionVerbMap[emotion] || verbPhrase;
    }
    ...
    // 4. 对话对象
    if (target) {
      parts.push(`（面向${target}）`);
    }

    // 5. 视觉配合描述
    const visualCues = [];
    if (emotion === '愤怒' || emotion === '挑衅') {
      visualCues.push('怒目圆睁，面部肌肉紧绷');
    } else if (emotion === '激动') {
      visualCues.push('眼神锐利，身体前倾');
    } else if (emotion === '紧张') {
      visualCues.push('眉头紧锁，呼吸急促');
    } else if (emotion === '平静') {
      visualCues.push('神态从容，目光平和');
    } else if (emotion === '威严') {
      visualCues.push('目光如电，气势沉稳');
    }

    if (type === '喊战') {
      visualCues.push('全身肌肉爆发，青筋暴起');
    }

    if (visualCues.length > 0) {
      parts.push(`，说话时${visualCues.join('，')}`);
    }

    // 6. 口型同步标记
    parts.push('. LIP_SYNC:YES');
  ...
  _extractActionDescription(shot, prompt) {
    // v6.5.65-P8-patch-008-fix: 优先从 shot.action 提取(已在上游注入)
    if (shot.action && typeof shot.action === 'string' && shot.action.trim().length > 5) {
      return shot.action.trim().substring(0, 100);
    }

    // 从 prompt 提取 ACTION: 标记(L1-L9 格式)
    ...
    // 回退:从 visualPrompt 提取前 100 字作为动作描述
    if (shot.visualPrompt && shot.visualPrompt.length > 10) {
      return shot.visualPrompt.substring(0, 100);
    }

    // 默认动作描述
    return shot.dialogue ? '角色自然讲述内容' : 'protagonist performs core action';
  }
  ...
  async stageFinalOutput(stages) {
    ...
    // ==== v6.6.2: 中文统一字段标准化输出（外部专家审计修复）====
    const standardization = 标准化最终输出(stages, {
      projectName: stages?.prd?.title || stages?.script?.title || '',
      title: stages?.prd?.title || '',
      topic: stages?.prd?.topic || ''
    });

    output.标准字段规范 = {
      版本: 'zh-shot-schema-v1',
      片头字段: [
        '镜头编号', '镜头类型', '场景名称', '主标题', '副标题', '台词',
        '旁白', '绑定定妆照', '镜头时间轴', '人物介绍卡片', '时长',
        '人物列表', '运镜', '灯光', '画面提示词', '音频', '负面约束'
      ],
      内容字段: [
        '镜头编号', '镜头类型', '场景名称', '主标题', '副标题', '台词',
        '旁白', '绑定定妆照', '镜头时间轴', '人物介绍卡片', '时长',
        '人物列表', '运镜', '灯光', '动作', '情绪阶段', '嘴部动作',
        '画面提示词', '音频', '负面约束'
      ]
    };

    output.标准镜头清单 = standardization.标准镜头清单;
    output.标准镜头校验 = standardization.校验结果;
    output.标准镜头全部通过 = standardization.全部通过;

    // ==== v6.6.2: 关键字段硬闸门 ====
    const criticalGate = 检查关键字段(output.标准镜头清单);
    output.关键字段闸门 = criticalGate;

    if (!criticalGate.passed) {
      this.log('STAGE-16', `❌ 关键字段闸门未通过 | ${criticalGate.errors.length}个错误`, 'error');
      criticalGate.errors.slice(0, 10).forEach(err => {
        this.log('STAGE-16', `  ❌ ${err}`, 'error');
      });
      throw new Error(`关键字段闸门未通过: ${criticalGate.errors.slice(0, 5).join(' | ')}`);
    }

    this.log('STAGE-16', `✅ 最终输出 | 镜头数: ${output.shots?.length || 0} | 中文标准镜头: ${output.标准镜头清单.length} | 关键字段闸门: 通过 | meta: ${output.meta?.title || 'N/A'} | 完整性验证: ${integrityResult.valid ? '通过' : '未通过'}`);
    return output;
  }
  ...
  _createFieldQualityLLM() {
    // v6.7.0: 为FieldQualityPipeline创建LLM客户端包装器
    const engine = this._createLLMEngine({ maxTokens: 2048 });
    return {
      chat: async (systemPrompt, userPrompt, temperature = 0.3) => {
        const response = await engine.chat(systemPrompt, userPrompt, { temperature });
  ...
  toStandardPrompt(shot, prompt) {
    ...
    // v6.6.3-fix-2: 替换 PromptTier-v2.0 标签为标准标签
    result = result.replace(/【主体】/g, '【角色】');
    result = result.replace(/【动态】/g, '【动作】');
    result = result.replace(/【空间】/g, '【场景】');
    result = result.replace(/【质控】/g, '【负面约束】');

    // v6.7.0: 替换旧标签为25字段标准标签
    result = result.replace(/【绑定定妆照】/g, '【定妆照】');
    result = result.replace(/【镜头时间轴】/g, '【时间轴】');
    result = result.replace(/【人物介绍卡片】/g, '【角色一致性】');
    // v6.7.0-patch: 统一带后缀标签（解决风险C：标签名不一致）
    result = result.replace(/【灯光\/照明】/g, '【灯光】');
    result = result.replace(/【色彩\/色调】/g, '【色彩】');

    // 1. 提取原始 prompt 的关键块（保护已有字段）
    const keyBlocks = 提取关键块(result);
    const has = 关键字段存在(result);

    // 2. 补关键缺口（统一使用中文标签，添加到开头以便 smartTrim 优先保留）
    const prefixPatches = [];

    // v6.7.0-patch: 字段默认模板（LLM未生成时的兜底，确保25字段全部存在且达标）
    const FIELD_DEFAULTS = {
      导演指令: '超写实纪录片质感, 无特效无科幻, 冷静专业科普基调, 8K影视级渲染, 真实人物比例',
      约束: 'Aspect ratio 16:9, Resolution 1920x1080, Format MP4, Frame rate 30fps, 横屏教育片规格, 单镜头单角色',
      基础: '8K, cinematic, photorealistic, highly detailed, sharp focus, professional color grading, volumetric lighting',
      负面约束: 'no text, no watermark, no blurry, no extra limbs, no deformed hands, no duplicate characters, no cartoon, no anime, no 3d render, no low quality, no dark shadows on face, no distorted facial features, no unnatural colors, no sci-fi elements, no fantasy elements',
      角色一致性: '画面中仅出现目标角色一人, 禁止分身, 禁止克隆, 禁止重复角色, 角色外观全程一致',
      明亮约束: 'bright well-lit, clear visibility, no dark shadows on face, adequate illumination, facial features clearly visible',
      角色约束: '画面中仅出现目标角色, 禁止重复角色入镜, 禁止角色分身, 角色位置分布自然避免重叠',
    };

    // 安全读取：兼容 shot.lighting 为字符串或对象
    const readLighting = (s) => {
      if (!s) return '';
      if (typeof s === 'string') return s;
      return s.string || s.keyLight?.effect || '';
    };
    const readCamera = (s) => {
      if (!s) return '';
      if (typeof s === 'string') return s;
      return s.string || s.description || `${s.shotSize || ''} ${s.movement || ''}`.trim();
    };

    const getField = (shotVal, defaultKey) => {
      const v = typeof shotVal === 'string' ? shotVal.trim() : (shotVal && shotVal.string ? shotVal.string.trim() : '');
      return v || (FIELD_DEFAULTS[defaultKey] || '');
    };

    // v6.7.0-patch: P0 致命级字段补洞（缺失则注入默认模板，不再跳过）
    if (!has.导演指令) {
      prefixPatches.push(`【导演指令】${getField(shot.directorStyle || shot.director_instruction, '导演指令')}`);
    }
    // 约束和基础原本是"硬编码注入"，现统一纳入25字段补洞
    if (!has.约束) {
      prefixPatches.push(`【约束】${getField(shot.constraint, '约束')}`);
    }
    if (!has.基础) {
      prefixPatches.push(`【基础】${getField(shot.baseline, '基础')}`);
    }
    if (!has.场景) {
      let scene = typeof shot.scene === 'string' ? shot.scene : (shot.scene?.name || '');
      if (typeof this._extractSceneDescription === 'function' && scene.length < 150) {
        scene = this._extractSceneDescription(shot, {});
      }
      prefixPatches.push(`【场景】${scene || FIELD_DEFAULTS['场景'] || '室内教育讲堂, 前景讲台, 中景人物, 背景健康教育海报与投影屏幕'}`);
    } else if (has.场景 && shot.scene) {
      let scene = typeof shot.scene === 'string' ? shot.scene : (shot.scene?.name || '');
      const sceneMatch = result.match(/【场景】([^【|；]*)/);
      const existingScene = sceneMatch ? sceneMatch[1].trim() : '';
      if (existingScene.length < 10 || (!existingScene.includes('【空间】') && typeof this._extractSceneDescription === 'function')) {
        scene = this._extractSceneDescription(shot, {});
        result = result.replace(/【场景】[^【|；]*/, `【场景】${scene}`);
      } else {
        const badTemplates = [
          'golden hour,clear sky,atmospheric haze,depth layers,foreground to background',
          'golden hour', 'atmospheric haze', 'depth layers', 'foreground to background'
        ];
        if (badTemplates.some(t => result.includes(t))) {
          result = result.replace(/【场景】[^【|；]*/, `【场景】${scene}`);
        }
      }
    }
    if (!has.灯光) {
      const lightStr = readLighting(shot.lighting);
      // v6.7.0-patch: 统一标签为【灯光】（去掉/照明后缀，解决下游提取不一致）
      prefixPatches.push(`【灯光】${lightStr || 'key light from front-right 45°, fill light left side, 5500K daylight, soft diffusion, 2:1 light ratio, even illumination on face'}`);
    }
    if (!has.运镜) {
      const camStr = readCamera(shot.cameraMovement || shot.camera_movement);
      if (camStr) prefixPatches.push(`【运镜】${camStr}`);
    }
    if (!has.角色 && Array.isArray(shot.characters) && shot.characters.length > 0) {
      prefixPatches.push(`【角色】${shot.characters.join(',')}`);
    }
    if (!has.动作 && shot.action) {
      prefixPatches.push(`【动作】${shot.action}`);
    }
    // v6.7.0-dialogue-patch: 支持 dialogueBlock 结构化对话，生成【对话指令】替代【台词】
    const dialogueBlock = this._buildDialogueBlock(shot);
    if (!has.对话指令 && dialogueBlock.text) {
      prefixPatches.push(`【对话指令】${this._renderDialogueInstruction(dialogueBlock, shot)}`);
    }
    // 向下兼容：如果 prompt 中已有【台词】但没有【对话指令】，将台词转化为对话指令
    if (!has.对话指令 && has.台词 && !has.台词被转换) {
      const existingDialogue = result.match(/【台词】([^【|]*)/)?.[1]?.trim();
      if (existingDialogue) {
        const compatBlock = this._buildDialogueBlock({ ...shot, dialogue: existingDialogue });
        prefixPatches.push(`【对话指令】${this._renderDialogueInstruction(compatBlock, shot)}`);
      }
    }
    if (!has.负面约束) {
      prefixPatches.push(`【负面约束】${getField(shot.negativePrompt || shot.negative, '负面约束')}`);
    }
    if (!has.定妆照 && shot.referenceImages && shot.referenceImages.length > 0) {
      const refs = shot.referenceImages
        .map(r => r.image_url?.url || r.url || '')
        .filter(Boolean)
        .join(',');
      if (refs) prefixPatches.push(`【定妆照】${refs}`);
    }
    if (!has.角色一致性) {
      prefixPatches.push(`【角色一致性】${getField(shot.characterProfile || shot.consistency, '角色一致性')}`);
    }

    // v6.7.0-patch: P1 核心级字段补洞（缺失注入默认/兜底）
    if (!has.构图 && shot.composition) {
      prefixPatches.push(`【构图】${shot.composition}`);
    } else if (!has.构图) {
      prefixPatches.push(`【构图】medium shot, subject centered-left, rule of thirds composition, clean background`);
    }
    if (!has.色彩 && (shot.colorPalette || shot.color_palette)) {
      // v6.7.0-patch: 统一标签为【色彩】（去掉/色调后缀）
      prefixPatches.push(`【色彩】${shot.colorPalette || shot.color_palette}`);
    } else if (!has.色彩) {
      prefixPatches.push(`【色彩】dominant warm neutral, accent soft blue, medium saturation, moderate contrast, professional educational tone`);
    }
    if (!has.景深 && (shot.depthOfField || shot.depth_of_field)) {
      prefixPatches.push(`【景深】${shot.depthOfField || shot.depth_of_field}`);
    } else if (!has.景深) {
      prefixPatches.push(`【景深】shallow depth of field, focus on subject face, background bokeh, professional cinematic look`);
    }
    if (!has.时间轴 && shot.duration) {
      const gs = shot._globalStartTime || 0;
      const ge = shot._globalEndTime || (gs + (shot.duration || 0));
      const fmt = (n) => `${String(Math.floor(n/60)).padStart(2,'0')}:${String(n%60).padStart(2,'0')}`;
      prefixPatches.push(`【时间轴】T${fmt(gs)}-T${fmt(ge)} / 时长:${shot.duration}s / 起始-发展-收尾三段式`);
    } else if (!has.时间轴) {
      prefixPatches.push(`【时间轴】T00:00-T00:03 establishing, T00:03-T00:06 dialogue, T00:06-T00:08 closing`);
    }
    if (!has.情绪 && (shot.emotionPhase || shot.mood)) {
      prefixPatches.push(`【情绪】${shot.emotionPhase || shot.mood}`);
    } else if (!has.情绪) {
      prefixPatches.push(`【情绪】calm professional, warm neutral tone, 5500K`);
    }
    if (!has.明亮约束) {
      prefixPatches.push(`【明亮约束】${getField(shot.brightConstraint || shot.bright_constraint, '明亮约束')}`);
    }
    if (!has.角色约束) {
      prefixPatches.push(`【角色约束】${getField(shot.characterConstraint || shot.character_constraint, '角色约束')}`);
    }

    // v6.7.0-patch: P2 增强级字段补洞（缺失注入兜底，确保字段存在）
    if (!has.服装 && shot.costume) {
      prefixPatches.push(`【服装】${shot.costume}`);
    } else if (!has.服装) {
      prefixPatches.push(`【服装】professional attire, fitted jacket, collared shirt, tailored trousers, leather shoes`);
    }
    if (!has.道具 && shot.props) {
      prefixPatches.push(`【道具】${shot.props}`);
    } else if (!has.道具) {
      prefixPatches.push(`【道具】lectern, projection screen, educational poster, data chart`);
    }
    if (!has.节奏 && shot.pacing) {
      prefixPatches.push(`【节奏】${shot.pacing}`);
    } else if (!has.节奏) {
      prefixPatches.push(`【节奏】overall steady, opening moderate, middle building, climax brief peak, ending calm`);
    }
    if (!has.音频 && shot.audio) {
      prefixPatches.push(`【音频】${shot.audio}`);
    } else if (!has.音频) {
      prefixPatches.push(`【音频】clear dialogue track, ambient room tone, subtle educational background music, no sudden peaks`);
    }

    // v6.7.0-patch: P3 可选级字段补洞
    if (!has.化妆 && shot.makeup) {
      prefixPatches.push(`【化妆】${shot.makeup}`);
    } else if (!has.化妆) {
      prefixPatches.push(`【化妆】natural makeup, even skin tone, no shine, professional on-camera look`);
    }
    if (!has.转场 && shot.transition) {
      prefixPatches.push(`【转场】${shot.transition}`);
    } else if (!has.转场) {
      prefixPatches.push(`【转场】hard cut`);
    }

    if (prefixPatches.length > 0) {
      result = prefixPatches.join(' | ') + ' | ' + result;
    }

    // 3. 裁剪到 3000 字符（v6.7.0: 从1500扩展到3000，优先保关键块）
  ...
  ensureFinalPromptStructure(shot, prompt) {
    ...
    // v6.7.0: 统一使用中文标签，消除中英混杂
    const patches = [];

    // P0 致命级补洞
    if (!has.导演指令 && shot.directorStyle) {
      patches.push(`【导演指令】${shot.directorStyle}`);
    }
    if (!has.场景 && shot.scene) {
      const scene = typeof shot.scene === 'string' ? shot.scene : (shot.scene?.name || '场景环境');
      patches.push(`【场景】${scene}`);
    }
    if (!has.灯光 && shot.lighting) {
      const li = shot.lighting;
      const lightStr = li.string || (li.keyLight?.effect) || '';
      if (lightStr) patches.push(`【灯光】${lightStr}`);
    }
    if (!has.运镜 && shot.cameraMovement) {
      const cm = shot.cameraMovement;
      const camStr = cm.string || cm.description || `${cm.shotSize || ''} ${cm.movement || ''}`.trim();
      if (camStr) patches.push(`【运镜】${camStr}`);
    }
    if (!has.角色 && Array.isArray(shot.characters) && shot.characters.length > 0) {
      patches.push(`【角色】${shot.characters.join(',')}`);
    }
    if (!has.动作 && (shot.action || shot.actionDescription)) {
      patches.push(`【动作】${safe(shot.action || shot.actionDescription)}`);
    }
    // v6.7.0-dialogue-patch: 支持 dialogueBlock 结构化对话
    if (!has.对话指令) {
      const dialogueBlock = this._buildDialogueBlock(shot);
      if (dialogueBlock.text) {
        patches.push(`【对话指令】${this._renderDialogueInstruction(dialogueBlock, shot)}`);
      }
    }
    // 向下兼容：如果 prompt 中有【台词】，转换为【对话指令】
    if (!has.对话指令 && has.台词) {
      const existingDialogue = result.match(/【台词】([^【|]*)/)?.[1]?.trim();
      if (existingDialogue) {
        const compatBlock = this._buildDialogueBlock({ ...shot, dialogue: existingDialogue });
        patches.push(`【对话指令】${this._renderDialogueInstruction(compatBlock, shot)}`);
      }
    }
    if (!has.负面约束 && shot.negativePrompt) {
      patches.push(`【负面约束】${shot.negativePrompt}`);
    }
    if (!has.定妆照 && shot.referenceImages && shot.referenceImages.length > 0) {
      const refs = shot.referenceImages
        .map(r => r.image_url?.url || r.url || '')
        .filter(Boolean)
        .join(',');
      if (refs) patches.push(`【定妆照】${refs}`);
    }
    if (!has.角色一致性 && shot.characterProfile) {
      patches.push(`【角色一致性】${shot.characterProfile}`);
    }

    // P1 核心级补洞
    if (!has.构图 && shot.composition) {
      patches.push(`【构图】${shot.composition}`);
    }
    if (!has.色彩 && shot.colorPalette) {
      patches.push(`【色彩】${shot.colorPalette}`);
    }
    if (!has.景深 && shot.depthOfField) {
      patches.push(`【景深】${shot.depthOfField}`);
    }
    if (!has.时间轴 && shot.duration) {
      const gs = shot._globalStartTime || 0;
      const ge = shot._globalEndTime || (gs + shot.duration);
      const fmt = (n) => `${String(Math.floor(n/60)).padStart(2,'0')}:${String(n%60).padStart(2,'0')}`;
      patches.push(`【时间轴】${fmt(gs)}-${fmt(ge)} / 时长:${shot.duration}s`);
    }
    if (!has.情绪 && (shot.emotionPhase || shot.mood)) {
      patches.push(`【情绪】${shot.emotionPhase || shot.mood}`);
    }
    if (!has.明亮约束 && shot.brightConstraint) {
      patches.push(`【明亮约束】${shot.brightConstraint}`);
    }
    if (!has.角色约束 && shot.characterConstraint) {
      patches.push(`【角色约束】${shot.characterConstraint}`);
    }

    // P2 增强级补洞
    if (!has.服装 && shot.costume) {
      patches.push(`【服装】${shot.costume}`);
    }
    if (!has.道具 && shot.props) {
      patches.push(`【道具】${shot.props}`);
    }
    if (!has.节奏 && shot.pacing) {
      patches.push(`【节奏】${shot.pacing}`);
    }
    if (!has.音频 && shot.audio) {
      patches.push(`【音频】${shot.audio}`);
    }

    // P3 可选级补洞
    if (!has.化妆 && shot.makeup) {
      patches.push(`【化妆】${shot.makeup}`);
    }
    if (!has.转场 && shot.transition) {
      patches.push(`【转场】${shot.transition}`);
    }

    if (patches.length > 0) {
      const prefix = patches.join(' | ') + ' | ';
      // v6.7.0: 3000字符上限
      const available = 3000 - prefix.length;
      if (result.length > available && available > 100) {
        const tailStart = Math.max(result.length - available + 50, result.length * 0.3);
        result = prefix + result.substring(tailStart);
      } else {
  ...
  smartTrim(prompt, maxLength, options = {}) {
    ...
    while ((match = markerPattern.exec(prompt)) !== null) {
      ...
      // v6.6.3-fix-2: 扩展支持【灯光】【渲染参数】【导演指令】
      const isL1L9Core = /^(照明|灯光|音频|内部渲染|渲染参数|导演|导演指令|负面约束|时间轴|情绪|运镜|台词|动作|角色|场景|视觉|叙事|独白|空间|主体|动态|风格|质控)$/.test(markerName);
...
情绪: ${scheme.emotion}`;
  ...
  enhanceAudioWithMethodology(audioText, shot) {
    ...
    if (shot && shot.scene && buildAudioDescription) {
      try {
        const fourLayerAudio = buildAudioDescription(shot, shot.cameraMovement?.timeline?.segments || []);
