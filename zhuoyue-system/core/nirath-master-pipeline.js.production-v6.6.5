...
const { spawn } = require('child_process');
  ...
  async _generateScriptCorePhase(input) {
    ...
    const llm = new LLMEngine({
      ...
      maxTokens: 3072,
      temperature: 0.6, // v6.6.5-fix: 降低随机性，提升指令遵循率
      topP: 0.9, // v6.6.5-fix: 略微降低
  ...
  _buildScriptCorePrompt(batch, core, world, input, batchIdx, totalBatches) {
    ...
    // ---------------- 系统角色 ----------------
    parts.push(`<system>`);
    parts.push(`你是一位资深健康科普视频剧本策划Agent。你的任务 ONLY 是根据给定的视频主题和场景要求，生成对应的剧本骨架。`);
    parts.push(`你严格遵守指令，绝不会把"原因分析"讲成"症状描述"，也绝不会修改用户给定的场景名称。`);
    parts.push(`</system>`);
    ...
    if (config.episodeIndex > 1) {
      ...
      parts.push(`<strict_rule>本集 ONLY 讲解"${config.topic || topic}"，禁止重复前期内容。</strict_rule>`);
    }
    parts.push(`</topic_context>`);

    // ---------------- 场景列表（场景名称保护） ----------------
    parts.push(`\n<scenes>`);
    parts.push(`<instruction>以下场景名称和描述是用户指定的，你必须原样使用名称，不得修改、优化或重写。</instruction>`);
    for (const scene of batch) {
      const requiredName = config.sceneNameMapping[scene.id] || scene.name;
      parts.push(`<scene id="${scene.id}" name="${requiredName}">`);
      parts.push(` <description>${scene.description || requiredName}</description>`);
    ...
    if (config.requiredKeywords && config.requiredKeywords.length > 0) {
      parts.push(`\n<required_topics>`);
      parts.push(`本集台词(dialogue)必须覆盖以下原因/机制方向（每段台词至少对应 1-2 个）：`);
    ...
    if (config.forbiddenKeywords && config.forbiddenKeywords.length > 0) {
      parts.push(`\n<forbidden_topics>`);
      parts.push(`以下内容是前期剧集已讲解的，本集严禁出现：`);
    ...
    parts.push(`错误 dialogue："横纹肌溶解最典型的症状就是肌肉疼痛、无力，尿液颜色加深..."`);
    parts.push(`注意：错误示例属于第一集内容，本集必须删除。`);
    ...
    // ---------------- 执行规则（肯定性优先） ----------------
    parts.push(`\n<rules>`);
    parts.push(`1. 必须直接使用 <scenes> 中提供的 scene 名称，禁止修改。`);
    parts.push(`2. 每个 dialogue 必须讲解"原因/机制"，不能讲解症状、检查、尿液颜色。`);
    parts.push(`3. 每个 dialogue 必须基于对应场景描述生成，禁止自行发挥。`);
    parts.push(`4. 必须在台词中自然融入 <required_topics> 中的关键词。`);
    parts.push(`5. 严禁出现 <forbidden_topics> 中的关键词。`);
    parts.push(`6. 如果某个场景描述要求讲"运动过度"，就必须讲运动过度，不能讲症状。`);
    parts.push(`</rules>`);

    // ---------------- 自我校验指令 ----------------
    parts.push(`\n<self_check>`);
    parts.push(`生成 JSON 后，请再次检查：\n- 是否所有 scene 名称与输入完全一致？\n- 是否任何 dialogue 包含症状/检查/尿液描述？\n- 是否每个 dialogue 都讲解了原因或机制？\n- 如果检查发现违规，请立即删除违规内容并重新生成。`);
  ...
  _checkTopicDeviation(dialogue, config) {
    ...
    // 偏离度评分：命中禁用词越多越偏离；无必需词也偏离
    let score = 0;
    if (config.forbiddenKeywords && config.forbiddenKeywords.length > 0) {
      score += forbiddenHits / Math.max(config.forbiddenKeywords.length, 1);
    }
    if (config.requiredKeywords && config.requiredKeywords.length > 0) {
      const requiredRatio = requiredHits / config.requiredKeywords.length;
      score += (1 - requiredRatio) * 0.3; // 缺少必需词贡献 30% 偏离
    }

    return { score: Math.min(score, 1), reasons, requiredHits, forbiddenHits };
  }
  ...
  _validateTopicAdherence(scenes, input) {
    ...
    const passed = failedScenes === 0 && avgDeviation < 0.25;
  ...
  async _autoCorrectScene(scene, config, llm) {
    const forbiddenList = (config.forbiddenKeywords || []).join('、');
    const requiredList = (config.requiredKeywords || []).join('、');

    const prompt = `<system>你是一位严格的内容修正Agent。你只修改台词(dialogue)，不修改场景名称和动作。</system>

<task>
原场景名称：${scene.scene}
原台词："""${scene.dialogue}"""

问题：当前台词偏离了主题"${config.topic}"，包含了前期剧集已讲的内容，或没有讲解原因/机制。

要求：
1. 重写 dialogue，必须围绕"${config.topic}"展开。
2. 自然融入以下关键词中的至少 2 个：${requiredList}。
3. 严禁出现以下内容：${forbiddenList}。
4. 保持原场景名称不变：${scene.scene}。
5. 保持科普视频的口语化、讲解风格，由主讲人直接口述。
6. 返回严格 JSON：{"dialogue": "修正后的台词"}
</task>`;
  ...
  async _autoCorrectScenes(scenes, validation, llm) {
    ...
    for (const scene of scenes) {
      ...
      if (detail && detail.score > 0.4) {
        this.log('STAGE-5', `🔧 自动修正场景 ${scene.id} (${scene.scene})，偏离度: ${detail.score.toFixed(2)}`);
  ...
  async stageCameraMovement(storyboard, fpvDecision, durations) {
    ...
    for (const shot of storyboard.shots) {
      ...
      if (shot.fpvRecommended || shot.shotType === 'fpv' || shot.type === 'fpv') {
        ...
        // v6.2-fix: 非FPV镜头使用v3完整运镜系统 — v6.6.4-root-fix: 教育片禁用Nirath v3运镜
        if (this.mode === 'nirath' && input?.videoType !== 'educational' && input?.filmType !== 'educational') {
          movement = this.generateV3CameraMovement(shot, timelineGenerator, sceneTypeToTransition, emotionToLighting, emotionToSpeedCurve);
        } else {
      ...
      try {
        const { result, driver, attempts } = await this.llmEnforcer.requireLLM(
          ...
          () => StagePrompts.STAGE_11_RENDER(storyboard.shots || [], stages, this.mode),
          {
            ...
            llmOptions: { maxTokens, temperature: 0.7 }
          }
        );
