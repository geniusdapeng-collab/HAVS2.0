...
let authorInfo;
try {
  authorInfo = require('./author-info.js');
} catch (e) {
  // 若模块缺失则静默跳过
  authorInfo = null;
}
...
// v6.7.0-dialogue-patch: 解析命令行参数
const args = process.argv.slice(2);
let cliProject = 'health-edu-ep01';
let cliFilmType = 'EDU';
let cliCP = 0.82;
for (let i = 0; i < args.length; i++) {
  if (args[i] === '--project' || args[i].startsWith('--project=')) {
    const val = args[i].includes('=') ? args[i].split('=')[1] : args[i + 1];
    if (val) cliProject = val;
  }
  if (args[i] === '--film-type' || args[i].startsWith('--film-type=')) {
    const val = args[i].includes('=') ? args[i].split('=')[1] : args[i + 1];
    if (val) cliFilmType = val;
  }
  if (args[i] === '--cp' || args[i].startsWith('--cp=')) {
    const val = args[i].includes('=') ? args[i].split('=')[1] : args[i + 1];
    if (val && !isNaN(parseFloat(val))) cliCP = parseFloat(val);
  }
}

const OUTPUT = path.join(WORKSPACE, 'output', cliProject);
...
// v6.6.7-fix: 自动清理旧输出（每次新任务前执行）
function cleanupOldOutput(outputDir) {
...
cleanupOldOutput(OUTPUT);
console.log('🧹 旧输出已清理');

// 根据项目配置角色定妆照和用户输入
let PORTRAIT_CONFIG, userInput;

if (cliProject === 'wukong-erlang') {
  // 孙悟空大战二郎神项目
  const WUKONG_DIR = path.join(WORKSPACE, 'characters/wukong/portraits');
  const ERLANG_DIR = path.join(WORKSPACE, 'characters/erlang-shen/portraits');
  const wukongFront = path.join(WUKONG_DIR, 'portrait-front.jpg');
  const erlangFront = path.join(ERLANG_DIR, 'portrait-front.jpg');
  PORTRAIT_CONFIG = {
    wukong: {
      front: wukongFront,
      threeQuarter: wukongFront, // fallback
      side: wukongFront, // fallback
      closeup: wukongFront, // fallback
    },
    'erlang-shen': {
      front: erlangFront,
      threeQuarter: erlangFront, // fallback
      side: erlangFront, // fallback
  ...
  PORTRAIT_CONFIG = {
    'chen-zhuo': {
      ...
      back: path.join(UNIFORM_DIR, 'portrait-cartoon-uniform-01.jpg')
    }
  };
  userInput = '科普视频。穿警服的陈卓女士讲解横纹肌溶解的症状及实验室检查。第一集，59-65秒，全写实风格，天花板般的创造力，电影级质感。陈卓一个人讲解，生动肢体语言，边走边介绍。三集系列，第一集不预告下一集。开头有片头主标题和副标题。';
}

// 验证定妆照存在
for (const [charName, angles] of Object.entries(PORTRAIT_CONFIG)) {
  for (const [angle, pPath] of Object.entries(angles)) {
    if (!fs.existsSync(pPath)) {
      console.warn('⚠️ 定妆照缺失:', charName, angle, pPath);
    }
  }
}
console.log('✅ 定妆照配置完成 | 项目:', cliProject);
...
async function run() {
  // v6.6.6: 首次运行欢迎信息
  const WELCOME_FLAG = path.join(WORKSPACE, '.install-welcome-shown');
  if (authorInfo && !fs.existsSync(WELCOME_FLAG)) {
    authorInfo.printWelcomeMessage();
    try {
      fs.writeFileSync(WELCOME_FLAG, 'Welcome message shown at ' + new Date().toISOString());
    } catch (e) { /* ignore */ }
  }
  ...
  const parsedRequirement = await parser.parse(userInput, {
    ...
    const input = buildPipelineInput(parsedRequirement, PORTRAIT_CONFIG, cliFilmType);
    ...
    const pipeline = new NirathMasterPipeline({
      ...
      outputDir: OUTPUT,  // v6.7.0-fix: 传递 outputDir 避免 checkpoint 散落在 /tmp
      useLLM: true,
      skipDirectorReview: false,
      skipScreenwriterOptimization: false,
      projectConfig: {
        requiredCharacters: Object.keys(input.characters || {}),
    ...
    // v6.6.6: 错误时提示联系作者
    if (authorInfo) {
      console.error(authorInfo.formatErrorWithContact(''));
    }
...
`;
  ...
  timelineData.forEach((shot, i) => {
    ...
    md += `**【全局时间定位】**: ${shot.timeStart} - ${shot.timeEnd}\n\n`;
    ...
    // 台词 - v6.6.8-patch2-fix: 竖杠替换为逗号，避免干扰音频生成
    const rawDialogue = shot.台词 || shot.dialogue || '(无台词)';
    const fixedDialogue = rawDialogue.replace(/\|/g, '，');
    md += `### 台词\n\n`;
    md += `${fixedDialogue}\n\n`;
    ...
    // 画面提示词 - v6.6.8-fix: 竖杠替换为分号
    // v6.6.8-patch2-fix: 服装锁定 + 外观锚定 + 定妆照引用
    let promptText = (shot.画面提示词 || shot.prompt || shot.visualPrompt || 'N/A').replace(/\|/g, '；');
    
    // 修复1: 服装锁定 - 将"穿职业套装"替换为警服描述
    promptText = promptText.replace(/穿职业套装/g, '穿藏蓝色警用制服，佩戴警帽、左胸警徽和警号、两肩肩章');
    
    // 修复2: 外观锚定 - 确保警服配饰描述存在
    if (!promptText.includes('警帽') && !promptText.includes('警徽')) {
      // 如果角色字段存在，在角色描述后追加警服锚定
      promptText = promptText.replace(/(【角色】[^；]+)/, '$1，穿藏蓝色警用制服，佩戴警帽、左胸警徽和警号、两肩肩章');
    }
    
    // 修复3: 定妆照引用 - 在prompt最前面添加"图片1"
    if (!promptText.includes('图片1')) {
      promptText = '图片1；' + promptText;
    }
...
function generateInternalTimeline(shot, duration) {
  ...
  // 🔥 v6.6.6-fix: 优先使用 Stage 9 生成的 v3 时间轴数据
  // v6.6.7-fix: 也从 原始字段 中读取（标准化后的镜头对象把 v3 数据存到了 原始字段）
  const raw = shot.原始字段 || {};
