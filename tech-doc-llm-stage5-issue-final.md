...
### 3.3 _llmGenerateScript 内部实现

```javascript
// _llmGenerateScript（约第2150行）
...
### 3.4 Phase A: _generateScriptCorePhase
...
### 3.5 Prompt 构建: _buildScriptCorePrompt
...
| 函数名 | 文件 | 行号 | 说明 |
|--------|------|------|------|
| `stageScriptGeneration` | nirath-master-pipeline.js | ~1950 | Stage 5 入口 |
| `_llmGenerateScript` | nirath-master-pipeline.js | ~2150 | LLM 生成主函数 |
| `_generateScriptCorePhase` | nirath-master-pipeline.js | ~2150 | Phase A 骨架生成 |
| `_buildScriptCorePrompt` | nirath-master-pipeline.js | ~2550 | Prompt 构建 |
| `_generateVisualPromptPhase` | nirath-master-pipeline.js | ~2350 | Phase B 视觉生成 |
| `_buildVisualPrompt` | nirath-master-pipeline.js | ~2750 | 视觉 Prompt 构建 |
...
关键片段：
```
用户要求我作为视频剧本策划Agent，为当前批次场景生成简洁、可直接用于视频制作的剧本骨架。
关键信息提取：
- 视频主题：为什么会发生横纹肌溶解，常见的原因分析
- 当前批次：3/6
- 场景列表只有一个场景：
  - id: S03
  - 名称: 药物和饮食因素
  - 核心指令：所有台词必须严格围绕"为什么会发生横纹肌溶解，常见的原因分析"展开
...
```
